...
lesson_1.HDLessonLayer = HDBaseLayer.extend({
  ...
  createLabelLoadingRes: function () {
    this.loadingResourceLabel = this.createTTFLabel(
      HDString.loadingResources,
      HDConstants.Sassoon_Medium,
      30,
      cc.color(0, 255, 0, 255),
      cc.p(this.getContentSize().width * 0.5, this.getContentSize().height * 0.7),
      this
    );
    this.loadingResourceLabel.setLocalZOrder(this.lessonZOrder);
    let bg = this.createButton(
      "res/LessonResources/emptyImage.png",
      "res/LessonResources/emptyImage.png",
      "",
      0,
      8888,
    ...
    bg.setScale(
      this.getContentSize().width / bg.getContentSize().width,
  ...
  passTouchToChildActivity: function (touch, event) {
    ...
    for (let obj of lesson_1.ref.handIconUI) {
      if (
        cc.rectContainsPoint(obj.getBoundingBox(), obj.getParent().convertToNodeSpace(touch.getLocation())) &&
        obj.isVisible() &&
        obj.getParent() &&
        obj.getParent().isVisible() &&
  ...
  createLogo: function (xMargin, yMargin) {
    ...
    logo.setPosition(
      this.getContentSize().width * xMargin + logo.getContentSize().width * 0.5,
      this.getContentSize().height * (1 - yMargin) - logo.getContentSize().height * 0.5
    );
  ...
  createRewardButton: function (xMargin, yMargin) {
    var starBGName = HDAppManager.isTeacherView
      ? lesson_1.resourcePath + lesson_1.config.graphicalAssets.starBgTeacher.name
      : lesson_1.resourcePath + lesson_1.config.graphicalAssets.starBgTeacher.name;
    var rewardBg = this.createButton(
      starBGName,
      null,
      null,
      null,
      lesson_1.Tag.rewardButton,
      cc.p(0, 0),
      this,
      this,
      starBGName
    );
    rewardBg.setPosition(
      this.getContentSize().width * (1 - xMargin) - rewardBg.getContentSize().width * 0.5,
      this.getContentSize().height * (1 - yMargin) - rewardBg.getContentSize().height * 0.5
    );
    rewardBg.setLocalZOrder(this.lessonZOrder);
    var star = this.addSprite(
      lesson_1.resourcePath + lesson_1.config.graphicalAssets.rewardStar.name,
      cc.p(rewardBg.width * 0.5, rewardBg.height * 0.5),
      rewardBg
    );
    star.setVisible(true);
    star.setTag(lesson_1.Tag.rewardStar);
    var rewardLabel = this.createTTFLabel(
      "",
      HDConstants.LondrinaSolid_Regular,
      15,
      cc.color(101, 64, 39, 255),
      cc.p(rewardBg.width * 0.5, rewardBg.height * 0.45),
      rewardBg
    );
    rewardLabel.setTag(lesson_1.Tag.rewardLabel);
    var isTouch = HDAppManager.isTeacherView;
    rewardBg.setEnabled(isTouch);
    lesson_1.ref.rewardAnimation = this.addSprite(
      "res/LessonResources/emptyImage.png",
      cc.p(lesson_1.ref.getContentSize().width * 0.5, lesson_1.ref.getContentSize().height * 0.5),
      this
    );
    lesson_1.ref.rewardAnimation.setLocalZOrder(this.lessonZOrder);
    if (HDAppManager.isTeacherView) {
      let position = cc.p(
        rewardBg.getPositionX() - rewardBg.width * 0.4,
        rewardBg.getPositionY() - rewardBg.height * 0.35
      );
  ...
  createRewardDropDown: function (position, marginY) {
    ...
    this.rewardDropDown = this.createColourLayer(
      HDConstants.Black,
      tempWidth,
    ...
    var selectedBG = this.addSprite(
      lesson_1.resourcePath + lesson_1.config.graphicalAssets.rewardDropDown.name,
      cc.p(0, position.y),
      this.rewardDropDown
    );
    ...
    var bottom = this.addSprite(
      lesson_1.resourcePath + lesson_1.config.graphicalAssets.bottomRewardDropDown.name,
      cc.p(0, selectedBG.getPositionY() - selectedBG.height * selectedBG.getScaleY()),
      this.rewardDropDown
    );
    ...
    var height =
      totalStudents > maxVisibleRewards ? cellHeight * maxVisibleRewards : (cellHeight + vPadding) * totalStudents;
    let scrollView = this.createScrollView(
      container.getContentSize(),
      container,
      cc.size(this.rewardDropDown.width, height),
      cc.SCROLLVIEW_DIRECTION_VERTICAL,
      cc.p(0, position.y - height - marginY),
      this.rewardDropDown
    );
  ...
  createStudentRewardCell: function (index, width, height) {
    ...
    var starIndex =
      lesson_1.ref.studentRewardList[index].starColor % lesson_1.config.graphicalAssets.studentRewards.length;
    var starButtonPng =
      lesson_1.resourcePath +
      lesson_1.config.graphicalAssets.studentRewards[
        cc.clampf(starIndex, 0, lesson_1.config.graphicalAssets.studentRewards.length - 1)
      ].name;
    ...
    if (HDAppManager.isTeacherView) this.handIconUI.push(button);
    var scoreTitle = this.createTTFLabel(
      lesson_1.ref.studentRewardList[index].rewards == 0
        ? "0"
        : lesson_1.ref.studentRewardList[index].rewards.toString(),
      HDConstants.LondrinaSolid_Regular,
      10,
      cc.color(101, 64, 39, 255),
      cc.p(button.getContentSize().width * 0.5, button.getContentSize().height * 0.45),
      button
    );
    scoreTitle.setTag(lesson_1.Tag.rewardLabel);
    this.createTTFLabel(
      lesson_1.ref.studentRewardList[index].name,
      HDConstants.LondrinaSolid_Regular,
      10,
      cc.color(101, 64, 39, 255),
      cc.p(cell.getContentSize().width * 0.5, cell.getContentSize().height * 0.15),
      cell
    );
    return cell;
  },
  ...
  reloadStudentRewardCell: function () {
    ...
    this.rewardDropDown
      .getChildByTag(lesson_1.Tag.bottomDrag)
      .setPositionY(rewardPanel.getPositionY() - rewardPanel.height * rewardPanel.getScaleY());
  },
  ...
  createScriptButton: function (xMargin, yMargin) {
    let messageButton = this.createButton(
      lesson_1.resourcePath + "btn_idle.png",
      lesson_1.resourcePath + "btn_selected.png",
      "",
      0,
      lesson_1.Tag.message,
      cc.p(0, 0),
      this,
      null
    );
    messageButton.setAnchorPoint(cc.p(0.5, 0.5));
    messageButton.setPosition(
      cc.p(
        this.getContentSize().width * xMargin + messageButton.getContentSize().width * 0.5,
        this.getContentSize().height * yMargin + messageButton.getContentSize().height * 0.5
      )
    );
    ...
    var scriptSprite = this.addSprite(
      lesson_1.resourcePath + "script_active_icon.png",
      cc.p(messageButton.width * 0.5, messageButton.height * 0.5),
      messageButton
    );
    scriptSprite.setScale(
      (messageButton.width * 0.5) / scriptSprite.width,
      (messageButton.height * 0.5) / scriptSprite.height
    );
  ...
  createReplayButton: function (messageButton) {
    //- Replay Button
    let messageReplayButton = this.createButton(
      lesson_1.resourcePath + "btn_idle.png",
      lesson_1.resourcePath + "btn_selected.png",
      "",
      0,
      lesson_1.Tag.replay,
      cc.p(messageButton.getPosition().x + messageButton.getContentSize().width * 1.15, messageButton.getPosition().y),
      this,
      null
    );
    ...
    var resetActiveSprite = this.addSprite(
      lesson_1.resourcePath + "reset_idle_icon.png",
      cc.p(messageReplayButton.width * 0.5, messageReplayButton.height * 0.5),
      messageReplayButton
    );
    resetActiveSprite.setTag(1);
    resetActiveSprite.setScale(
      (messageReplayButton.width * 0.5) / resetActiveSprite.width,
      (messageReplayButton.height * 0.5) / resetActiveSprite.height
    );
  ...
  createShortcutButton: function (xMargin, yMargin) {
    ...
    let shortcutButton = this.createButton(
      lesson_1.resourcePath + "btn_idle.png",
      lesson_1.resourcePath + "btn_selected.png",
      "",
      0,
      lesson_1.Tag.shortcutHelp,
      cc.p(0, 0),
      this,
      null
    );
    shortcutButton.setPosition(
      cc.p(
        this.getContentSize().width * (1 - xMargin) - shortcutButton.getContentSize().width * 0.5,
        this.getContentSize().height * yMargin + shortcutButton.getContentSize().height * 0.5
      )
    );
    ...
    var tipSprite = this.addSprite(
      lesson_1.resourcePath + "tip_active_icon.png",
      cc.p(shortcutButton.width * 0.5, shortcutButton.height * 0.5),
      shortcutButton
    );
    tipSprite.setScale(
      (shortcutButton.width * 0.5) / tipSprite.width,
      (shortcutButton.height * 0.5) / tipSprite.height
    );
    tipSprite.setTag(1);
  ...
  createMouseControlButton: function (shortcutButton) {
    let mouseControlButton = this.createButton(
      lesson_1.resourcePath + "btn_idle.png",
      lesson_1.resourcePath + "btn_selected.png",
      "",
      0,
      lesson_1.Tag.mouseControl,
      cc.p(
        shortcutButton.getPosition().x - shortcutButton.getContentSize().width * 1.15,
        shortcutButton.getPosition().y
      ),
      this,
      null
    );
    ...
    var mouseControlSprite = this.addSprite(
      lesson_1.resourcePath + "cursor_idle_icon.png",
      cc.p(mouseControlButton.width * 0.5, mouseControlButton.height * 0.5),
      mouseControlButton
    );
    mouseControlSprite.setScale(
      (mouseControlButton.width * 0.5) / mouseControlSprite.width,
      (mouseControlButton.height * 0.5) / mouseControlSprite.height
    );
  ...
  teacherUI: function () {
    ...
    this.createTimer(
      lesson_1.resourcePath + lesson_1.config.graphicalAssets.lessonTimer[0].name,
      lesson_1.ref.lessonZOrder + 1,
      lesson_1.Tag.lessonTimer
    ); // lessonTimer
    this.createTimer(
      lesson_1.resourcePath + "timer_1_minute.png",
      lesson_1.ref.lessonZOrder + 1,
      lesson_1.Tag.timerTag
    ); // activity timer
  },
  ...
  activityBar: function () {
    ...
    let maxWidth =
      (activityCount > maxItemsCountWithoutScroll ? maxItemsCountWithScroll : maxItemsCountWithoutScroll) * buttonWidth;
    ...
    scrollView.setPosition(
      cc.p(
        this.getContentSize().width * 0.5 - cc.clampf(width, 0, maxWidth) * 0.5,
    ...
    if (activityCount > maxItemsCountWithoutScroll) {
      var leftNavigation = this.createButton(
        lesson_1.resourcePath + "leftArrow.png",
        lesson_1.resourcePath + "leftArrow.png",
        "",
        0,
        lesson_1.Tag.leftArrow,
        cc.p(this.getContentSize().width * 0.2, this.getContentSize().height * 0.85),
        this
      );
      ...
      //arrowRight
      var rightNavigation = this.createButton(
        lesson_1.resourcePath + "rightArrow.png",
        lesson_1.resourcePath + "rightArrow.png",
        "",
        0,
        lesson_1.Tag.rightArrow,
        cc.p(this.getContentSize().width * 0.8, this.getContentSize().height * 0.85),
        this
      );
      ...
      if (
        scrollView.getContentOffset().x > scrollView.maxContainerOffset().x - 1 &&
        scrollView.getContentOffset().x < scrollView.maxContainerOffset().x + 1
      ) {
        leftNavigation.setOpacity(100);
        leftNavigation.setTouchEnabled(false);
      } else if (
        scrollView.getContentOffset().x > scrollView.minContainerOffset().x - 1 &&
        scrollView.getContentOffset().x < scrollView.minContainerOffset().x + 1
      ) {
    ...
    for (let i = 0; i < activityCount; i++) {
      let activityIcon = this.addSprite(
        "res/LessonResources/lesson_1_activityDone.png",
        cc.p(15 + i * buttonWidth, scrollView.getContentSize().height * 0.5),
        scrollView
      );
      ...
      let blackDot1 = this.addSprite(
        "res/LessonResources/lesson_1_blackDot.png",
        cc.p(activityIcon.getPositionX() + activityIcon.getContentSize().width * 0.65, activityIcon.getPositionY()),
        scrollView
      );
      blackDot1.setTag(lesson_1.Tag.blackDot);
      let blackDot2 = this.addSprite(
        "res/LessonResources/lesson_1_blackDot.png",
        cc.p(blackDot1.getPositionX() + blackDot1.getContentSize().width * 1.5, activityIcon.getPositionY()),
        scrollView
      );
      blackDot2.setTag(lesson_1.Tag.blackDot);
    }
    if (activityCount > maxItemsCountWithoutScroll) {
      leftNavigation.setPosition(
        cc.p(
          scrollView.getPosition().x - scrollView.width * 0.05,
          scrollView.getPosition().y + scrollView.getContentSize().height * 0.5
        )
      );
      rightNavigation.setPosition(
        cc.p(
          scrollView.getPosition().x + scrollView.width * 1.05,
          scrollView.getPosition().y + scrollView.getContentSize().height * 0.5
        )
      );
    }
  ...
  updateActivityBar: function () {
    ...
    if (this.getChildByTag(lesson_1.Tag.activityBox)) this.getChildByTag(lesson_1.Tag.activityBox).setVisible(false);
  ...
  scrollActivityBar: function (direction) {
    ...
    let finalOffset = cc.p(
      cc.clampf(currentOffset.x + moveBy, activityBar.minContainerOffset().x, activityBar.maxContainerOffset().x),
      currentOffset.y
    );
    if (
      finalOffset.x > activityBar.maxContainerOffset().x - 5 &&
      finalOffset.x < activityBar.maxContainerOffset().x + 5
    ) {
      left.setOpacity(100);
      left.setTouchEnabled(false);
    } else if (
      finalOffset.x > activityBar.minContainerOffset().x - 5 &&
      finalOffset.x < activityBar.minContainerOffset().x + 5
    ) {
  ...
  createActivityDialog: function () {
    ...
    let activityName = this.createTTFLabel(
      "Magic Hat",
      HDConstants.LondrinaSolid_Regular,
      28,
      cc.color(101, 64, 39, 255),
      cc.p(layer.getContentSize().width * 0.5, layer.getContentSize().height * 0.6),
      layer
    );
    activityName.setTag(lesson_1.Tag.activityName);
    activityName.setDimensions(cc.size(layer.getContentSize().width * 0.9, 0));
    let activityTime = this.createTTFLabel(
      "Time: 1 Min",
      HDConstants.Sassoon_Regular,
      24,
      HDConstants.Black,
      cc.p(layer.getContentSize().width * 0.5, layer.getContentSize().height * 0.4),
      layer
    );
    activityTime.setTag(lesson_1.Tag.activityTime);
    let playButton = this.createButton(
      lesson_1.resourcePath + "play.png",
      lesson_1.resourcePath + "play.png",
      "",
      0,
      lesson_1.Tag.playButton,
      cc.p(layer.getContentSize().width * 0.5, layer.getContentSize().height * 0.25),
      layer,
      this
    );
  ...
  showActivityDialog: function (activityNumber) {
    ...
    activityBox
      .getChildByTag(lesson_1.Tag.activityName)
      .setString(lesson_1.config.activityGame[activityNumber].activityName);
    if (HDAppManager.loadedResIndex.indexOf(activityNumber) == -1) {
      ...
      activityBox
        .getChildByTag(lesson_1.Tag.activityTime)
        .setString("Time: " + lesson_1.config.activityGame[activityNumber].allocatedTime + " Min");
    }
    activityBox.setVisible(true);
    let position = this.convertToNodeSpace(activityLayer.convertToWorldSpace(button.getPosition()));
    activityBox.setPosition(
      cc.p(
        position.x - activityBox.getContentSize().width * activityBox.getScale() * 0.5,
        position.y - activityBox.getContentSize().height * activityBox.getScale() * 1.15
      )
    );
  },
  ...
  studentsPanel: function () {
    ...
    let scrollView = this.createScrollView(
      container.getContentSize(),
      container,
      cc.size(this.getContentSize().width * 0.2, 9 * this.studentCellSize.height * 2),
      cc.SCROLLVIEW_DIRECTION_VERTICAL,
      cc.p(this.getContentSize().width * 0.7, this.getContentSize().height * 0.3),
      this
    );
    container.setContentSize(
      cc.size(this.getContentSize().width * 0.2, cellGap * (count + 1) + count * cellSize.height)
    );
    ...
    this.studentPanelBG = new cc.Scale9Sprite(
      lesson_1.resourcePath + "student_panel_bg.png",
      null,
      cc.rect(40, 40, 40, 40)
    ); //this.addSprite(lesson_1.resourcePath + "student_panel_bg.png", );
    this.studentPanelBG.setAnchorPoint(cc.p(0, 0));
    this.studentPanelBG.setPosition(
      cc.p(
        scrollView.getPositionX() - this.studentCellSize.width * 0.0625,
        scrollView.getPositionY() - this.studentCellSize.width * 0.0625
      )
    );
    ...
    scrollView.setLocalZOrder(this.lessonZOrder);
    this.studentPanelBG.setScale(
      (container.getContentSize().width + this.studentCellSize.width * 0.25) /
        this.studentPanelBG.getContentSize().width,
      (container.getContentSize().height + this.studentCellSize.width * 0.25) /
        this.studentPanelBG.getContentSize().height
    );
  },
  ...
  studentsPreviewPanel: function () {
    ...
    var scrollView = new cc.ScrollView(cc.size(bgSprite.width, this.studentPreviewPanelHeight), bgSprite);
    scrollView.setDirection(cc.SCROLLVIEW_DIRECTION_HORIZONTAL);
    scrollView.setTouchEnabled(true);
    scrollView.setBounceable(false);
  ...
  studentPreviewCellCallback: function (sender, type) {
    ...
    switch (type) {
      case ccui.Widget.TOUCH_ENDED: {
        let prevSelectedCell = scrollView
          .getContainer()
          .getChildren()
          .find((item) => item.getName() === this.activePreviewStudentName);
  ...
  createStudentCell: function (container, index) {
    let nameBg = this.createButton(
      "res/LessonResources/btn_student_enable.png",
      "res/LessonResources/btn_student_enable.png",
      "",
      0,
      lesson_1.Tag.studentListStart + index,
      cc.p(0, 0),
      container,
      this
    );
    ...
    nameBg.setScale(0.5);
    nameBg.name = this.createTTFLabel(
      lesson_1.ref.studentList[index],
      HDConstants.Sassoon_Regular,
      20,
      HDConstants.Black,
      cc.p(nameBg.getContentSize().width * 0.45, nameBg.getContentSize().height * 0.5),
      nameBg
    );
    nameBg.name.setAnchorPoint(cc.p(0.5, 0.5));
    this.studentCellSize = cc.size(
      nameBg.getContentSize().width * nameBg.getScale(),
      nameBg.getContentSize().height * nameBg.getScale()
    );
  ...
  toggleMouseControlPanel(status) {
    ...
    if (!this.isTeacherView || !this.mouseControl || !scrollView) {
      if (this.studentPanelBG) this.studentPanelBG.setVisible(false);
      return;
    }
    let count = lesson_1.ref.studentList.length;
    let cellSize = this.studentCellSize;
    if (status === undefined) status = scrollView.isVisible();
    if (!status) {
      ...
      this.studentPanelBG.setScale(
        (container.getContentSize().width + this.studentCellSize.width * 0.125) /
          this.studentPanelBG.getContentSize().width,
        (container.getContentSize().height + this.studentCellSize.width * 0.125) /
          this.studentPanelBG.getContentSize().height
      );
      let j = 0;
      ...
      for (var i = 0; i < count; ++i) {
        ...
        if (!student) {
          ...
          student.setPosition(
            container.getContentSize().width * 0.5,
            container.getContentSize().height - (cellSize.height * i * 1.2 + this.studentCellSize.height * 0.5)
          );
        }
    ...
    mouseButtonSprite.setScale(
      (mouseControlButton.width * 0.5) / mouseButtonSprite.width,
      (mouseControlButton.height * 0.5) / mouseButtonSprite.height
    );
  },
    ...
    switch (type) {
      case ccui.Widget.TOUCH_ENDED:
        ...
        cc.sys.localStorage.setItem(
          "localRoomInfo",
          JSON.stringify({ roomId: HDAppManager.roomId, activeStudents: lesson_1.ref.activeStudentList })
        );
    }
  },
  ...
  launchActivity: function (index) {
    if (index == this.activityIdx) return;
    ...
    let layer = eval(
      "new " +
        lesson_1.config.activityGame[this.activityIdx].namespace.toString() +
        "." +
        lesson_1.config.activityGame[this.activityIdx].layerName.toString() +
        "()"
    );
    ...
    if (this.isTeacherView) {
      ...
      rewardButton.setVisible(
        HDAppManager.isTeacherView && lesson_1.config.activityGame[this.activityIdx].externalRewards
      );
    }
  },
  /**
  ...
  subscribeToEvents: function () {
    if (SocketManager.socket == null || !SocketManager.isSocketConnected()) {
      ...
      SocketManager.subscribeEvent("error", this.onSocketError);
    }
  },
  ...
  socketListener: function (res) {
    switch (res.eventType) {
      ...
      case HDSocketEventType.REWARD_RECEIVED:
        if (!HDAppManager.isTeacherView) lesson_1.ref.onRewardReceive(res.data);
        break;
    }
  ...
  onRoomCreated: function (res) {
    ...
    if (HDAppManager.isTeacherView) {
      ...
      this.emitSocketEvent(HDSocketEventType.LESSON_START_AT, {
        roomId: res.roomId,
        userType: HDAppManager.isTeacherView ? 1 : 2,
      });
    }

    if (!this.currentActivity) {
      this.startActivityTimer(lesson_1.config.activityGame[0].allocatedTime * 60);
      HDAppManager.setActivityStartTime(new Date().getTime());
      this.launchActivity(0);
    }
  ...
  onStudentStatusUpdate: function (res) {
    ...
    if (res.data && res.roomId && parseInt(res.roomId) != HDAppManager.roomId) return;
    if (
      lesson_1.ref.activeStudent &&
      res.users.map((user) => user.userName).indexOf(lesson_1.ref.activeStudent.name.getString()) == -1
    ) {
      lesson_1.ref.activeStudent = null;
    }
  ...
  onStudentTurn: function (res) {
    if (!lesson_1.ref.isTeacherView) return;
    if (res.data && res.roomId && parseInt(res.roomId) != HDAppManager.roomId) return;
  ...
  onRoomJoined: function (data) {
    ...
    let roomInfo = cc.sys.localStorage.getItem("localRoomInfo");
    if (roomInfo) {
      roomInfo = JSON.parse(roomInfo);
      if (roomInfo.roomId == data.roomId) {
        this.activeStudentList = roomInfo.activeStudents;
      }
    }
  ...
  onLessonCompleted: function (res) {
    var alertText = HDAppManager.isTeacherView
      ? "Lesson has been marked as completed."
      : "Lesson has been marked as completed by Teacher.";
  ...
  emitSocketEvent: function (type, data) {
    SocketManager.emitCutomEvent(lesson_1.socketEventKey.singleEvent, {
      eventType: type,
  ...
  showScript: function (val) {
    ...
    scriptButton
      .getChildByTag(1)
      .setTexture(lesson_1.resourcePath + (val ? "script_active_icon.png" : "script_idle_icon.png"));
  ...
  showTip: function (isAvailable, sender) {
    ...
    tipButton
      .getChildByTag(1)
      .setTexture(lesson_1.resourcePath + (isAvailable ? "tip_active_icon.png" : "tip_idle_icon.png"));
  ...
  mouseControlStatus: function () {
    let scrollView = lesson_1.ref.getChildByTag(lesson_1.Tag.studentScrollView);
    if (scrollView) scrollView.setVisible(lesson_1.ref.mouseControl);
    ...
    if (mouseButton) {
      ...
      if (!lesson_1.ref.mouseControl) {
        ...
        mouseButtonSprite.setScale(
          mouseButton.width / mouseButtonSprite.width,
          mouseButton.height / mouseButtonSprite.height
        );
      }
  ...
  updateActivity(index) {
    ...
    index = index % (lesson_1.config.activityGame.length + 1);
    if (this.isTeacherView) {
      this.emitSocketEvent(HDSocketEventType.LAUNCH_ACTIVITY, JSON.stringify({ lesson: "1", activityIndex: index }));
      HDAppManager.setActivityStartTime(new Date().getTime());
      this.startActivityTimer(lesson_1.config.activityGame[index].allocatedTime * 60); // when we switch to new activity
    }
    //
    //check If Resource Loaded
    //
    if (!HDAppManager.loadedResIndex.includes(index)) {
  ...
  checkIfMouseClickedOnMouseControlPanel: function (event) {
    if (
      this.getChildByTag(lesson_1.Tag.studentScrollView) &&
      this.getChildByTag(lesson_1.Tag.studentScrollView).isVisible() &&
      cc.rectContainsPoint(
        this.studentPanelBG.getBoundingBox(),
        this.studentPanelBG.parent.convertToNodeSpace(event.getLocation())
      )
    ) {
  ...
  checkIfMouseClickedOutSideRewardPanel: function (event) {
    if (this.isRewardPanelVisible) {
      ...
      if (
        cc.rectContainsPoint(
          rewardPanel.getBoundingBox(),
          this.rewardDropDown.convertToNodeSpace(event.getLocation())
        ) ||
        cc.rectContainsPoint(buttonDrag.getBoundingBox(), this.rewardDropDown.convertToNodeSpace(event.getLocation()))
      ) {
        // console.log("chicmic");
      } else {
        if (this.isTeacherView) {
  ...
  checkIfMouseClickedOnPreviewPanel: function (event) {
    return false;
    let studentPreviewPanelScrollView = this.getChildByTag(lesson_1.Tag.studentPreviewScrollView);
    if (
      studentPreviewPanelScrollView &&
      studentPreviewPanelScrollView.isVisible() &&
      cc.rectContainsPoint(studentPreviewPanelScrollView.getBoundingBox(), event.getLocation())
    ) {
  ...
  updateMouseIcon(event) {
    ...
    for (let obj of lesson_1.ref.handIconUI) {
      if (
        cc.rectContainsPoint(obj.getBoundingBox(), obj.getParent().convertToNodeSpace(event.getLocation())) &&
        obj.isVisible() &&
        obj.getParent() &&
        obj.getParent().isVisible() &&
        obj.getParent().getParent() &&
    ...
    if (activityCursorFound) {
      ...
      if (lesson_1.ref.texture) {
        if (lesson_1.ref.texture == "none") {
          ...
          cc.$("#gameCanvas").style.cursor =
            "url" + "('" + "AsyncActivity/" + lesson_1.ref.texture + "')" + "," + "auto";
        }
      }
    }
    if (cursorFound) {
  ...
  moveStudentPanel: function (event) {
    if (
      this.isMouseOnMouseControlPanel &&
      this.getChildByTag(lesson_1.Tag.studentScrollView) &&
      this.getChildByTag(lesson_1.Tag.studentScrollView).isVisible()
    ) {
      let mouseLocation = this.studentPanelBG.parent.convertToNodeSpace(event.getLocation());
      if (!this.mousePosDiff) {
        this.mousePosDiff = cc.p(
          mouseLocation.x - this.studentPanelBG.getPositionX(),
      ...
      if (scrollList && scrollList.isVisible()) {
        scrollList.setPosition(
          this.studentPanelBG.getPositionX() + this.studentCellSize.width * 0.0625,
          this.studentPanelBG.getPositionY() + this.studentCellSize.width * 0.0625
        );
      }
    }
  },
  ...
  checkIfStudentSwitchTurnDisabled: function (obj) {
    if (obj.tag == lesson_1.Tag.onlineButton) {
      let activityLayer = lesson_1.ref.currentActivity;
      if (activityLayer && activityLayer.isTurnSwitchingBlocked) {
        if (activityLayer.isTurnSwitchingBlocked()) return true;
      }
    }
    return false;
  },
  ...
  activityInfoOnMouseHover(event) {
    ...
    let activityBox = cc.rect(
      activityBar.getPositionX(),
      activityBar.getPositionY(),
      activitySize.width,
      activitySize.height
    );
    if (activityLayer) {
      ...
      for (let i = 0; i < lesson_1.config.activityGame.length; i++) {
        ...
        if (
          cc.rectContainsPoint(button.getBoundingBox(), locationInNode) &&
          cc.rectContainsPoint(activityBox, buttonPosition)
        ) {
          found = true;
      ...
      if (
        !found &&
        !cc.rectContainsPoint(
          lesson_1.ref.getChildByTag(lesson_1.Tag.activityParent).getBoundingBox(),
          lesson_1.ref.convertToNodeSpace(event.getLocation())
        ) &&
        !cc.rectContainsPoint(
          lesson_1.ref.getChildByTag(lesson_1.Tag.activityBox).getBoundingBox(),
          lesson_1.ref.convertToNodeSpace(event.getLocation())
        )
      ) {
      ...
      case ccui.Widget.TOUCH_ENDED:
        switch (Tag) {
          case lesson_1.Tag.playButton:
            ...
            if (this.isRewardAnimationCompleted) this.updateActivity(this.playActivity);
            break;
          ...
          case lesson_1.Tag.rewardButton:
            if (lesson_1.ref.rewardAnimation.getNumberOfRunningActions() > 0 || this.studentList.length == 0) return;
            else {
              sender.setTouchEnabled(false);
              var data = { studentUserName: lesson_1.ref.studentRewardList[0].name, roomId: HDAppManager.roomId };
              lesson_1.ref.isMultiPlayer
                ? lesson_1.ref.showStudentRewardPanel()
                : lesson_1.ref.sendRewardInfoToServer(data, sender, 8);
            }
            break;
        }
        break;
  ...
  startRewardAnimation: function (index, rewardCount, sender) {
    let animation = HDUtility.runFrameAnimation(
      "res/LessonResources/AnimationFrames/reward_star_appear/reward_star_appear_",
      22,
      0.083,
      ".png",
      1
    );
    let animation2 = HDUtility.runFrameAnimation(
      "res/LessonResources/AnimationFrames/reward_star_track/reward_star_track_",
      14,
      0.083,
      ".png",
      1
    ...
    var callBack = new cc.CallFunc(function () {
      ...
      lesson_1.ref.rewardAnimation.runAction(
        cc.spawn([animation2, moveAction, cc.sequence([cc.delayTime(0.581), starSprite])])
      );
    });
  ...
  getDataForRewardAnimation: function (index) {
    let data = null;
    let arrayAngle = [7.5, 14, 20, 26, 32, 38, 44.5, 50, 0];
    let arrayPositionX = [
      cc.winSize.width * 0.442,
  ...
  setResetButtonActive: function (flag) {
    ...
    resetButton
      .getChildByTag(1)
      .setTexture(lesson_1.resourcePath + `${flag ? "reset_active_icon.png" : "reset_idle_icon.png"}`);
  ...
  createScriptBubble: function () {
    ...
    scriptMessageHolderNode.setPosition(
      cc.p(
        messageButton.x - messageButton.width * 0.5,
        messageButton.y + messageButton.height * 0.5 + this.getContentSize().height * 0.01
      )
    );
    ...
    var scriptBubbleBottomSprite = this.addSprite(
      lesson_1.resourcePath + "script_bubble_bottom.png",
      cc.p(0, 0),
      scriptMessageHolderNode
    );
    scriptBubbleBottomSprite.setTag(lesson_1.Tag.scriptBubbleBottom);
    scriptBubbleBottomSprite.setAnchorPoint(0, 0);

    //
    let scriptBubbleTopSprite = this.addSprite(
      lesson_1.resourcePath + "script_bubble_top.png",
      cc.p(0, 0),
      scriptMessageHolderNode
    );
    ...
    //
    let scriptBubbleMiddleSprite = this.addSprite(
      lesson_1.resourcePath + "script_bubble_middle.png",
      cc.p(0, scriptBubbleBottomSprite.y + scriptBubbleBottomSprite.height * 0.95),
      scriptMessageHolderNode
    );
    scriptBubbleMiddleSprite.setAnchorPoint(cc.p(0, 0));
    scriptBubbleMiddleSprite.setTag(lesson_1.Tag.script_bubble_middle);

    let swallowTouchButton = new ccui.Button(
      lesson_1.resourcePath + "bubble_speech_buttom_left_small.png",
      lesson_1.resourcePath + "bubble_speech_buttom_left_small.png"
    );
  ...
  createTipBubble: function () {
    ...
    tipMessageHolderNode.setPosition(
      cc.p(
        shortcutButton.x + shortcutButton.width * 0.5,
        shortcutButton.y + shortcutButton.height * 0.5 + this.getContentSize().height * 0.01
      )
    );
    ...
    var tipBubbleBottomSprite = this.addSprite(
      lesson_1.resourcePath + "tip_bubble_bottom.png",
      cc.p(tipMessageHolderNode.width, 0),
      tipMessageHolderNode
    );
    ...
    let tipBubbleTopSprite = this.addSprite(
      lesson_1.resourcePath + "tip_bubble_top.png",
      cc.p(tipMessageHolderNode.width, 0),
      tipMessageHolderNode
    );
    ...
    let tipBubbleMiddleSprite = this.addSprite(
      lesson_1.resourcePath + "tip_bubble_middle.png",
      cc.p(tipMessageHolderNode.width, tipBubbleBottomSprite.y + tipBubbleBottomSprite.height * 0.95),
      tipMessageHolderNode
    );
    ...
    let swallowTouchButton = new ccui.Button(
      lesson_1.resourcePath + "bubble_speech_buttom_right_small.png",
      lesson_1.resourcePath + "bubble_speech_buttom_right_small.png"
    );
    swallowTouchButton.setAnchorPoint(cc.p(0, 0));
    swallowTouchButton.setOpacity(0);
  ...
  updateScript: function (message) {
    ...
    let maxLabelWidth = scriptBubbleBottomSprite.width * 0.8;
    let maxHeight = cc.winSize.height * 0.6;
    let yPadding = this.height * 0.015; // 0.01
    let paddingWidth = 0;
    for (var i in parseMessage) {
      let labelRef = new HDCustomLabel(
        parseMessage[i].insert,
        HDConstants.Sassoon_Regular,
        18.75,
        cc.size(maxLabelWidth, 0),
        parseMessage[i].attributes,
        paddingWidth
      );
    ...
    scriptBubbleBottomSprite.setPosition(
      cc.p(scriptBubbleBottomSprite.getPositionX(), scriptBubbleBottomSprite.getPositionY())
    );
    scriptBubbleMiddleSprite.setScaleY(cc.clampf(totalInnerHeight, 0, maxHeight) / scriptBubbleMiddleSprite.height);
    scriptBubbleTopSprite.setPositionY(
      scriptBubbleMiddleSprite.y + scriptBubbleMiddleSprite.height * scriptBubbleMiddleSprite.scaleY * 0.99
    );
    scriptMessageHolderNode.setContentSize(
      cc.size(
        scriptBubbleBottomSprite.width,
        scriptBubbleBottomSprite.height +
          scriptBubbleMiddleSprite.height * scriptBubbleMiddleSprite.scaleY +
          scriptBubbleTopSprite.height
      )
    );
    let swallowTouchButton = scriptMessageHolderNode.getChildByTag(lesson_1.Tag.scriptBubbleSwallowTouch);
    swallowTouchButton.setScale(
      scriptBubbleBottomSprite.width / swallowTouchButton.width,
      scriptMessageHolderNode.height / swallowTouchButton.height
    );
  ...
  updateTip: function (message) {
    ...
    let yPadding = this.height * 0.015; // 0.01
    let maxLabelWidth = tipBubbleBottomSprite.width * 0.8;
    let maxHeight = cc.winSize.height * 0.6;
    var paddingWidth = 0;
    for (var i in parseMessage) {
      let labelRef = new HDCustomLabel(
    ...
    tipBubbleTopSprite.setPositionY(
      tipBubbleMiddleSprite.y + tipBubbleMiddleSprite.height * tipBubbleMiddleSprite.scaleY * 0.99
    );
    tipMessageHolderNode.setContentSize(
      cc.size(
        tipBubbleBottomSprite.width,
        tipBubbleBottomSprite.height +
          tipBubbleMiddleSprite.height * tipBubbleMiddleSprite.scaleY +
          tipBubbleTopSprite.height
      )
    );
  ...
  addSocketConnectionStatusLabel: function () {
    var label = this.createTTFLabel(
      ...
      22,
      HDConstants.Green,
      cc.p(this.width * 0.65, this.height * 0.97),
      this
    );
  ...
  lessonCloseButton: function () {
    let closeLesson = this.createButton(
      "res/LessonResources/lesson_1_studentNameBG.png",
      "res/LessonResources/lesson_1_studentNameBG.png",
      "Close lesson",
      20,
      lesson_1.Tag.lessonComplete,
      cc.p(this.getContentSize().width * 0.08, this.getContentSize().height * 0.15),
      this
    );
  ...
  createTimer: function (fileName, zOrder, tag) {
    ...
    var timerLabel = this.createTTFLabel(
      "",
      HDConstants.Sassoon_Medium,
      25,
      cc.color(116, 84, 33, 255),
      cc.p(timerSprite.width * 0.5, timerSprite.height * 0.4),
      timerSprite
    );
  ...
  performTimerAction: function (slideOut = true, timerTag) {
    ...
    slideInFinalPos_X =
      (logoSprite.x + (leftArrow ? leftArrow.x : this.getChildByTag(lesson_1.Tag.activityParent).x)) * 0.5;
  ...
  addRoomIdLabel: function (roomId) {
    ...
    let label = this.createTTFLabel(
      "Room id: " + roomId,
      HDConstants.Sassoon_Regular,
      22,
      HDConstants.Black,
      cc.p(this.getContentSize().width * 0.2, this.getContentSize().height * 0.97),
      this
    );
    label.setLocalZOrder(this.lessonZOrder);
    label.setTag(41578);
        ...
        for (let index = 0; index < HDAppManager.config.activityGame.length; index++) {
          ...
          if (roomData && roomData.activity == activityFromConfig.namespace) {
            ...
            if (
              ...
              currentActivity.config.properties &&
              currentActivity.config.properties.namespace == roomData.activity
            ) {
  ...
  setupTableView: function () {
    var availableLabel = this.createTTFLabel(
      "Available Rooms",
      HDConstants.Sassoon_Regular,
      24,
      cc.color(0, 0, 0, 0),
      cc.p(this.getContentSize().width * 0.5, this.getContentSize().height * 0.75),
      this.roomLayer
    );
  ...
  setupButtonLayer: function () {
    this.buttonLayer = this.createColourLayer(
      cc.color(0, 0, 0, 0),
      this.getContentSize().width * 0.5,
      this.getContentSize().height * 0.5,
      cc.p(this.getContentSize().width * 0.25, this.getContentSize().height * 0.25),
      this.roomLayer,
      100
    );
    var roleLabel = this.createTTFLabel(
      "Are you teacher? ",
      HDConstants.Sassoon_Medium,
      32,
      cc.color(0, 0, 0, 255),
      cc.p(this.buttonLayer.getContentSize().width * 0.5, this.buttonLayer.getContentSize().height * 0.75),
      this.buttonLayer
    );
    var teacherButton = this.createButton(
      "res/LessonResources/lesson_1_studentNameBG.png",
      "res/LessonResources/lesson_1_studentNameBG.png",
      "YES",
      20,
      lesson_1.Tag.teacherButton,
      cc.p(this.buttonLayer.getContentSize().width * 0.25, this.buttonLayer.getContentSize().height * 0.5),
      this.buttonLayer,
      this,
      "res/LessonResources/lesson_1_studentNameBG.png"
    );
    teacherButton.setTitleColor(cc.color(0, 0, 0));
    var studentButton = this.createButton(
      "res/LessonResources/lesson_1_studentNameBG.png",
      "res/LessonResources/lesson_1_studentNameBG.png",
      "NO",
      20,
      lesson_1.Tag.studentButton,
      cc.p(this.buttonLayer.getContentSize().width * 0.75, this.buttonLayer.getContentSize().height * 0.5),
      this.buttonLayer,
      this,
      "res/LessonResources/lesson_1_studentNameBG.png"
    );
  ...
  setupLoadingLayer: function (message) {
    ...
    var roleLabel = this.createTTFLabel(
      ...
      32,
      cc.color(0, 0, 0, 255),
      cc.p(this.buttonLayer.getContentSize().width * 0.5, this.buttonLayer.getContentSize().height * 0.5),
      this.buttonLayer
    );
  },

  addEditBox: function () {
    this.roleIdEditBoxLayer = this.createColourLayer(
      cc.color(0, 0, 0, 0),
      this.getContentSize().width * 0.5,
      this.getContentSize().height * 0.5,
      cc.p(this.getContentSize().width * 0.25, this.getContentSize().height * 0.25),
    ...
    var textTitle = HDAppManager.isTeacherView
      ? "Create new room or enter room id to Join Room"
    ...
    var roleLabel = this.createTTFLabel(
      ...
      cc.color(0, 0, 0, 255),
      cc.p(
        this.roleIdEditBoxLayer.getContentSize().width * 0.5,
        this.roleIdEditBoxLayer.getContentSize().height * 0.75
      ),
      this.roleIdEditBoxLayer
    );
    this.roomIdEditBox = this.createEditBox(
      "res/LessonResources/editBoxbackgroundWhite.png",
      cc.p(this.editBoxLayer.getContentSize().width * 0.5, this.editBoxLayer.getContentSize().height * 0.5),
      "  Enter room Id",
      lesson_1.Tag.roomIdEditBox,
      cc.EDITBOX_INPUT_MODE_NUMERIC,
      this.roleIdEditBoxLayer
    );
    this.roomIdEditBox.setDelegate(this);
    this.roomIdEditBox.setFontColor(cc.color(0, 0, 0, 255));
    var bgSprite = this.addSprite(
      "res/LessonResources/editBoxbackground.png",
      cc.p(this.roomIdEditBox.x, this.roomIdEditBox.y),
      this.roleIdEditBoxLayer
    );
    bgSprite.setLocalZOrder(-12);
    bgSprite.setScale(
      (this.roomIdEditBox.width * 1.1) / bgSprite.width,
      (this.roomIdEditBox.height * 1.1) / bgSprite.height
    );

    if (!HDAppManager.isTeacherView) {
      this.joinUserNameButton = this.createButton(
        "res/LessonResources/doneEnable.png",
        "res/LessonResources/doneEnable.png",
        "Join Room",
        24,
        lesson_1.Tag.joinRoomButton,
        cc.p(
          this.roleIdEditBoxLayer.getContentSize().width * 0.5,
          this.roleIdEditBoxLayer.getContentSize().height * 0.3
        ),
        this.roleIdEditBoxLayer,
        this,
        "res/LessonResources/doneDisable.png"
      );
      ...
      this.joinUserNameButton = this.createButton(
        ...
        lesson_1.Tag.joinRoomButton,
        cc.p(
          this.roleIdEditBoxLayer.getContentSize().width * 0.35,
          this.roleIdEditBoxLayer.getContentSize().height * 0.3
        ),
        this.roleIdEditBoxLayer,
        this,
      ...
      this.createRoomButton = this.createButton(
        ...
        "Create Room",
        20,
        lesson_1.Tag.createRoomButtom,
        cc.p(
          this.roleIdEditBoxLayer.getContentSize().width * 0.65,
          this.roleIdEditBoxLayer.getContentSize().height * 0.3
        ),
        this.roleIdEditBoxLayer,
        this,
  ...
  userNameEditBox: function () {
    this.editBoxLayer = this.createColourLayer(
      cc.color(0, 0, 0, 0),
      this.getContentSize().width * 0.5,
      this.getContentSize().height * 0.5,
      cc.p(this.getContentSize().width * 0.25, this.getContentSize().height * 0.25),
    ...
    var roleLabel = this.createTTFLabel(
      ...
      32,
      cc.color(0, 0, 0, 255),
      cc.p(this.editBoxLayer.getContentSize().width * 0.5, this.editBoxLayer.getContentSize().height * 0.75),
      this.editBoxLayer
    );
    this.userNameEditBox = this.createEditBox(
      "res/LessonResources/editBoxbackgroundWhite.png",
      cc.p(this.editBoxLayer.getContentSize().width * 0.5, this.editBoxLayer.getContentSize().height * 0.5),
      "  Enter username",
      lesson_1.Tag.userNameEditBox,
    ...
    var bgSprite = this.addSprite(
      "res/LessonResources/editBoxbackground.png",
      cc.p(this.userNameEditBox.x, this.userNameEditBox.y),
      this.editBoxLayer
    );
    bgSprite.setLocalZOrder(-12);
    bgSprite.setScale(
      (this.userNameEditBox.width * 1.1) / bgSprite.width,
      (this.userNameEditBox.height * 1.1) / bgSprite.height
    );
    this.doneUserNameButton = this.createButton(
      "res/LessonResources/doneEnable.png",
      "res/LessonResources/doneEnable.png",
      "Done",
      20,
      lesson_1.Tag.userNameEnterButton,
      cc.p(this.editBoxLayer.getContentSize().width * 0.5, this.editBoxLayer.getContentSize().height * 0.3),
      this.editBoxLayer,
      this,
      "res/LessonResources/doneDisable.png"
    );
  ...
  setRoomsContent: function () {
    ...
    let baseColorLayer = this.createColourLayer(
...
lesson_1.HDRoomDataCell = cc.TableViewCell.extend({
  ...
  createUI: function (idx, data, parent) {
    ...
    let labelCardText = new cc.LabelTTF(
      "Room " + data,
      HDConstants.Sassoon_Regular,
      16,
      cc.size(200, 0),
  ...
  startTimer: function (time, delegate, timerType) {
    ...
    this.interval = setInterval(() => {
      ...
      if (this.progress >= time) {
        ...
        if (
          this.delegate &&
          this.timerType == lesson_1.TimerType.activityTimer &&
          !this.delegate.isLessonTimerOnScreen
        ) {
        ...
        this.progress > 0 &&
        this.progress < time &&
        (this.progress % Math.trunc(time * 0.25) == 0 || this.progress == Math.trunc(time * 0.9))
      ) {
...
lesson_1.HDLessonScene = cc.Scene.extend({
  ...
  onEnter: function () {
    this._super();
    var layer = new lesson_1.HDLessonLayer(); //ACTIVITY_HUNGRY_FLUPE_1.HungryFlupeLayer(); //

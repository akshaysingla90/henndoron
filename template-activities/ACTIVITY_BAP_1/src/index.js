...
ACTIVITY_BAP_1.BackgroundAndPenLayer = HDBaseLayer.extend({
    ...
    onExit : function () {
      ...
      // SocketManager.emitCutomEvent("SingleEvent", {'eventType':HDSocketEventType.UPDATE_ROOM_DATA,'roomId': HDAppManager.roomId ,'data':  {"roomId":HDAppManager.roomId,"roomData": {"activity":   ACTIVITY_BAP_1.config.properties.namespace,"data":{ 'dataArray':{}, 'slideIndex': 0}} }},null);
    ...
    updateRoomData: function() {
        SocketManager.emitCutomEvent("SingleEvent", {'eventType':HDSocketEventType.UPDATE_ROOM_DATA,'roomId': HDAppManager.roomId ,'data':  {"roomId":HDAppManager.roomId,"roomData": {"activity":   ACTIVITY_BAP_1.config.properties.namespace,"data":{ 'dataArray':this.lineSyncData, 'slideIndex': this.curImageIdx},  "activityStartTime" : HDAppManager.getActivityStartTime()} }},null);
    },
    ...
    updateStudentInteraction: function(username, status){
        if(this.isTeacherView){
            this.emitSocketEvent(HDSocketEventType.GAME_MESSAGE, {"eventType": ACTIVITY_BAP_1.socketEventKey.STUDENT_INTERACTION, "data":{"userName": username, "status": status} });
        }
    },
    ...
    onMouseMove : function(event){
        ...
        ACTIVITY_BAP_1.ref.parent.logLabel.setString("move " + ACTIVITY_BAP_1.ref.isStudentInteractionEnable);
        ...
        if(!ACTIVITY_BAP_1.ref)
          ...
          switch (res.eventType) {
              case HDSocketEventType.DISABLE_STUDENT_INTERACTION:
                  // ACTIVITY_BAP_1.ref.disableChildInteraction();
                  break;
              ...
              case HDSocketEventType.RECEIVE_GRADES:
                  // ACTIVITY_BAP_1.ref.receiveGrades();
                  break;
        ...
        switch (type) {
            case ccui.Widget.TOUCH_ENDED:
                    if(sender.rgb){
                        ...
                        this.erasing = false;
                        this.MouseTextureUrl = (ACTIVITY_BAP_1.config.preLoaded ? ACTIVITY_BAP_1.spriteBasePath : "AsyncActivity/" + ACTIVITY_BAP_1.spriteBasePath ) + "cursor_pencil.png";
                    }else{
                        this.erasing = true;
                        this.MouseTextureUrl = (ACTIVITY_BAP_1.config.preLoaded ? ACTIVITY_BAP_1.spriteBasePath : "AsyncActivity/" + ACTIVITY_BAP_1.spriteBasePath ) + "cursor_eraser.png";
                    }
                break;
        }
    },
    ...
    paintSelf: function(pointArray){
        ...
        for(let i = 0; i < pointArray.length; ++i) {
                ...
                this.drawPointArr.push({"lastPoint": this.lastPoint, "curPoint":this.curPoint, "color": {"r":this.selectedColor.r, "g": this.selectedColor.g, "b": this.selectedColor.b, "a": this.selectedColor.a}, "userName": HDAppManager.username, "tag":tag, "slideIndex":this.curImageIdx});
    ...
    performPaint: function(last, cur, color, userName, tag, imageIndex){
        ...
        if(this.isTeacherView) {
            if( this.allLineInfo[userName][imageIndex].length == 1){
                // this.getChildByTag(ACTIVITY_BAP_1.Tag.Clear).loadTextureNormal(ACTIVITY_BAP_1.spriteBasePath + ACTIVITY_BAP_1.config.graphicalAssets.clear_enable.name);
                // this.getChildByTag(ACTIVITY_BAP_1.Tag.Clear).setTouchEnabled(true);
            ...
            this.lineDataForsync[index].data.push({"tag": node.getTag(), "source": JSON.stringify(last), "dest": JSON.stringify(cur), "color": {"r":color.r, "g": color.g, "b": color.b, "a": color.a}, "slideIndex": imageIndex, "userName": userName});
        }
    ...
    updateSlide : function(increase){
        ...
        if(this.isTeacherView){
            ...
            this.emitSocketEvent(HDSocketEventType.GAME_MESSAGE, {'eventType':ACTIVITY_BAP_1.socketEventKey.CHANGE_SLIDE, 'data':{'index': this.curImageIdx}});
            this.triggerScriptByCurrentSlideIdx();
        }
        this.showPreview();
    },
    ...
    emitDrawModeEvent:function(){
        this.emitSocketEvent(HDSocketEventType.GAME_MESSAGE, {"eventType": ACTIVITY_BAP_1.socketEventKey.DRAW_MODE, "data":{"status": this.sharedMode} });
    },

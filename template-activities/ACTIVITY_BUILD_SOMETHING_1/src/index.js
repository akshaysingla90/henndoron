...
ACTIVITY_BUILD_SOMETHING_1.BuildSomething = HDBaseLayer.extend({
    ...
    onEnter: function () {
        ...
        cc.loader.loadJson("res/Activity/ACTIVITY_BUILD_SOMETHING_1/config.json", function (error, config) {
            ...
            // ACTIVITY_BUILD_SOMETHING_1.ref.gameState = ACTIVITY_BUILD_SOMETHING_1.ref.isTeacherView ? ACTIVITY_BUILD_SOMETHING_1.events.TEACHER_DEMO : false;
          ...
          //  ACTIVITY_BUILD_SOMETHING_1.ref.playCharacterAnimation();
    ...
    onMouseUp: function(event) {
        ...
        if(ACTIVITY_BUILD_SOMETHING_1.ref.tableView) {
            ACTIVITY_BUILD_SOMETHING_1.ref.tableView._dragging  = this.isStudentInteractionEnable;
            // ACTIVITY_BUILD_SOMETHING_1.ref.tableView.setTouchEnabled( ACTIVITY_BUILD_SOMETHING_1.ref.isStudentInteractionEnable);
            var loc = ACTIVITY_BUILD_SOMETHING_1.ref.convertToNodeSpace(event.getLocation());
       ...
       // cc.log("Match",location);
        ...
        if (this.clickedItem) {
            ...
            if (result.isIsolatedNode) {
                ...
                // console.log("before connectionObject  ", connectionObject, "clickedItem ",clickedItem, "attach result ", attachResult);
                if(connectionObject && clickedItem && connectionObject.userData && clickedItem && attachResult.snapIdxOfPlacedObj != -1 && attachResult.snapIdxOfDraggedObj !=  -1) {
                    // Placed Obj
                    // // console.log(" inside user data ", connectionObject.userData.snappingPoints);
                    connectionObject.userData.snappingPoints.splice(attachResult.snapIdxOfPlacedObj, 1);
                    // // console.log("after inside user data ", connectionObject.userData.snappingPoints);
                    clickedItem.snappingPoints.splice(attachResult.snapIdxOfDraggedObj, 1);
                }
                // console.log("after connectionObject  ", connectionObject, "clickedItem ",clickedItem, "attach result ", attachResult);
                let completeMovement = cc.callFunc(function () {
                    console.log("===========  ");
                    if (ACTIVITY_BUILD_SOMETHING_1.ref.tableView) {
                        ACTIVITY_BUILD_SOMETHING_1.ref.assembledList.push(ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem);
                        // console.log("(ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem ", (ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem));
                        ACTIVITY_BUILD_SOMETHING_1.ref.moveObject(ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getPosition(), "", ACTIVITY_BUILD_SOMETHING_1.events.STOP);
    ...
    updateUserDataForSync: function (nodeName, position, size, data, correct, isNew, index) {
        if (!isNew && index != -1) {
            ...
            currentNode.data.push({"position": position, "size": size, "scaleX":ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getScaleX(),"scaleY":ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getScaleY(), "nodeData": data});
        } else {
            this.userDataForSync.push({
                "nodeName": nodeName,
                "isCorrect": correct,
                "data": [{"position": position, "size":size, "scaleX":ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getScaleX(),"scaleY":ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getScaleY(), "nodeData": data}]
            })
        }
        this.sendUserData();
        for (let object of ACTIVITY_BUILD_SOMETHING_1.config.gameInfo.assembleObjectInfo){
            for(let assembledObj of this.userDataForSync){
                ...
                //    console.log(" inside all compledted ", assembledObj, object);
                    ...
                    if (ACTIVITY_BUILD_SOMETHING_1.ref.isTeacherView && ACTIVITY_BUILD_SOMETHING_1.ref.gameState == ACTIVITY_BUILD_SOMETHING_1.gameState.TEACHER_DEMO) {
                        this.emitSocketEvent(HDSocketEventType.GAME_MESSAGE, {
                            ...
                            'data': {'userName': HDAppManager.userName, "assembledObj": assembledObj, "animationInfo": object}
                        });
    ...
    attachPoint: function (loc, index) {
        ...
        // this.findAvilableSnappingPoints(connectionObjectData.snappingPoints);
        ...
        // ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.addChild(this.createColourLayer(cc.p(0,0,0,100),ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getContentSize().width,ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getContentSize().height,cc.p(0,0),ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem,200));
        if (connectionObject && connectionObjectData && clickedItemData) {
            for (let index = 0; index < connectionObjectData.snappingPoints.length; index++) {
                ...
                for (let currIndex = 0; currIndex < clickedItemData.snappingPoints.length; currIndex++) {
                    // cc.log("CurrentObject",clickedItemData.snappingPoints[currIndex]);
                    ...
                    // var blue = this.createColourLayer(cc.color(0,0,255,255),10,10,currentPointInWorldSpace,this,1000);
                    ...
                    if (attachFromLocation && attachToLocation) {
                        if (previousDistance > distance) {
                            ...
                            // console.log("OriginalLength ",clickedItemData.snappingPoints.length);
                            // console.log("originalPoint SnappingPoint",  clickedItemData.snappingPoints);
                            // console.log("originalPoint Index",  currIndex);
                            // console.log("attachedItemSnapPoint ", attachedItemSnapPoint);
                        }
                    } else {
                        attachFromLocation = currentPointInWorldSpace;
                        attachToLocation = pointInWorldSpace;
    ...
    allPartsConnectedSuccessfully : function(allPartsInfo, animationInfo){
        ...
        let obj2 = new cc.DrawNode();
        let obj3 = new cc.DrawNode();
        let obj4 = new cc.DrawNode();
    ...
    updateStudentInteraction: function(username, status){
        if(this.isTeacherView){
            this.emitSocketEvent(HDSocketEventType.GAME_MESSAGE, {"eventType": ACTIVITY_BUILD_SOMETHING_1.socketEventKey.STUDENT_INTERACTION, "data":{"userName": username, "status": status} });
        }
    },
    ...
    gameEvents: function (res) {
        // console.log(' Game events', res);
        switch (res.eventType) {
            case ACTIVITY_BUILD_SOMETHING_1.socketEventKey.MOVE_POSITION:
                if(!this.isTeacherView && this.gameState !=  ACTIVITY_BUILD_SOMETHING_1.gameState.TEACHER_DEMO)
                    return;
                // cc.log("Position1",res.data.position);
            ...
            case ACTIVITY_BUILD_SOMETHING_1.socketEventKey.GAME_STATE:
                ...
                // console.log("game state",  ACTIVITY_BUILD_SOMETHING_1.ref.gameState);
            ...
            case ACTIVITY_BUILD_SOMETHING_1.socketEventKey.USER_DATA:
                ...
                ACTIVITY_BUILD_SOMETHING_1.ref.updateRoomData();
                // ACTIVITY_BUILD_SOMETHING_1.ref.userDataForSync = res.data.UIData;
                // ACTIVITY_BUILD_SOMETHING_1.ref.allUserData = [{"userName":res.userName,"data": res.data.UIData}];
                break;
    ...
    onUpdateStudentInteraction: function(params){
        if(params.userName == HDAppManager.username){
            ...
            // ACTIVITY_BUILD_SOMETHING_1.ref.tableView.setTouchEnabled(params.status);
        }
    },
    ...
    moveClickedObject: function (data) {
        ...
        // cc.log("moveClickedObject");
        switch (data.event) {
            case ACTIVITY_BUILD_SOMETHING_1.events.START:
                // cc.log("Position3 -",data.position);
                ACTIVITY_BUILD_SOMETHING_1.ref.moveCellObject( this.tableView.cellAtIndex(data.index), data.position);
                // ACTIVITY_BUILD_SOMETHING_1.ref.tableCellTouched(this.tableCellAtIndex, );
                break;
            case ACTIVITY_BUILD_SOMETHING_1.events.MOVE:
                // cc.log("Position2 -",data.position);
                if (ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem) {
                  //  if(ACTIVITY_BUILD_SOMETHING_1.ref.game == ACTIVITY_BUILD_SOMETHING_1.gameState.TEACHER_DEMO)
            ...
            case ACTIVITY_BUILD_SOMETHING_1.events.STOP:
                if (ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem) {
                    // cc.log("Position1 -",data.position);
    ...
    createCardQueue: function(data) {
        ...
        ACTIVITY_BUILD_SOMETHING_1.ref.draggableObject.length = 0;
        if(ACTIVITY_BUILD_SOMETHING_1.ref.tableView) {
            // console.log("dismental object ", this.dismantledObject);
            ACTIVITY_BUILD_SOMETHING_1.ref.tableView.reloadData();
        }
    },
    ...
    resetScreen: function () {
        ...
        let dismantledObjects = ACTIVITY_BUILD_SOMETHING_1.config.dismantledObject;
    ...
    createUIForData: function(userName, data,parent) {
        for(let index = 0; index < data.length; index++) {
            ...
            for(let indexNode = 0; indexNode < currentNodeData.length; indexNode++) {
                ...
                // console.log("gameref sceen", ACTIVITY_BUILD_SOMETHING_1.ref.gameState);
                switch(ACTIVITY_BUILD_SOMETHING_1.ref.gameState) {
                    case ACTIVITY_BUILD_SOMETHING_1.gameState.TEACHER_DEMO:
        ...
        // console.log("assembled list", ACTIVITY_BUILD_SOMETHING_1.ref.assembledList);
    },
    ...
    updateScrollOffset : function(isLeft) {
        ...
        // console.log("is student avilabel", ACTIVITY_BUILD_SOMETHING_1.ref.isStudentInteractionEnable);
    ...
    setCardContent: function () {
        ...
        //carouselBaseImage.setTag(ACTIVITY_BUILD_SOMETHING_1.Tag.carouselBaseImage);
        ...
        let leftButton = this.createButton(ACTIVITY_BUILD_SOMETHING_1.spriteBasePath+ACTIVITY_BUILD_SOMETHING_1.config.graphicalAssets.leftMoveButton.idleImage,ACTIVITY_BUILD_SOMETHING_1.spriteBasePath+ACTIVITY_BUILD_SOMETHING_1.config.graphicalAssets.leftMoveButton.pushedImage,"",8,ACTIVITY_BUILD_SOMETHING_1.Tag.leftButton,cc.p(position.x, position.y),baseColorLayer,this,ACTIVITY_BUILD_SOMETHING_1.spriteBasePath+ACTIVITY_BUILD_SOMETHING_1.config.graphicalAssets.leftMoveButton.idleImage);
        ...
        // let reloadTable = cc.callFunc(function () {
        //     if (ACTIVITY_BUILD_SOMETHING_1.ref.tableView) {
        //         //////cc.log("Reload1");
        //         // ACTIVITY_BUILD_SOMETHING_1.ref.tableView.reloadData();
        //     }
        // });
        // this.runAction(cc.sequence(cc.delayTime(0.05), reloadTable));
        // ACTIVITY_BUILD_SOMETHING_1.ref.tableView.setTouchEnabled(this.isStudentInteractionEnable);
        this.manageUserInteraction();
    },

    manageUserInteraction: function () {
        if (!ACTIVITY_BUILD_SOMETHING_1.ref.isTeacherView) {
            // ACTIVITY_BUILD_SOMETHING_1.ref.tableView.setTouchEnabled(ACTIVITY_BUILD_SOMETHING_1.ref.isStudentInteractionEnable);
        }
    ...
    getWidthOfCarousel: function () {
        ...
        //
        // let cellWidthSize = this.carouselHeight * ACTIVITY_BUILD_SOMETHING_1.ref.dismantledObject.length;
        //
    ...
    tableCellSizeForIndex: function (table, idx) {
        ...
        // //console.log("width", width, idx,  table.getViewSize().height);
        return cc.size(width, table.getViewSize().height);
    },
    ...
    tableCellTouched: function (table, cell) {
        ...
        //     return;
        // }
        // this.moveCellObject(cell);
        // ACTIVITY_BUILD_SOMETHING_1.ref.tableView.setTouchEnabled(false);
        // return;
        },
    tableCellHighlight: function (table, cell) {
        //console.log(" tableCVellHighlight ", cell);
        ACTIVITY_BUILD_SOMETHING_1.ref.mousePressedDown = true;
    },
    ...
    moveCellObject: function (cell, position) {
        ...
        let completedAnimation = cc.callFunc(function () {
            if(ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem)
                return;
            // if (ACTIVITY_BUILD_SOMETHING_1.ref.isCellSelected(cell.cellData)) {
            // } else {
                ...
                // ACTIVITY_BUILD_SOMETHING_1.ref.mousePressedDown = true;
               // //console.log("moveCellObject ", cell.isVisible());
                // //console.log(" cell.getPosition() ",  cell.getPosition(),  cell.parent.getPosition(),  cell.parent.parent.getPosition(),  cell.parent.parent.parent.getPosition());
            ...
            //    //console.log(" final position", ACTIVITY_BUILD_SOMETHING_1.ref.convertToNodeSpace(finalPosition));
                ...
                let aspectRatio = ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getContentSize().width / ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getContentSize().height;
                var scaleFactorX = (cell.getContentSize().height * aspectRatio) / ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getContentSize().width;
                var scaleFactorY = cell.getContentSize().height / ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getContentSize().height;
                ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.setScaleX(0.5);
                ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.setScaleY(0.5);
                // //console.log(" finalPosition ", finalPosition,  ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getContentSize().width * ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getScale());
                // ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.setPosition(cc.p(finalPosition.x + ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getContentSize().width * 0.5 ,  finalPosition.y + ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getContentSize().height * 0.5));
    ...
    showResultInfo : function (data) {
        // console.log("show result info", data);
        if(data.length > 0) {
            this.parent.setResetButtonActive(true);
    ...
    showPreviewToAll : function (res) {
        ...
        var bg =this.getChildByTag(ACTIVITY_BUILD_SOMETHING_1.Tag.studentPreviewLayer);
           ...
           var label = this.createTTFLabel(res.userName, HDConstants.Sassoon_Regular, 40, HDConstants.Black, cc.p(position.x, position.y), bg);
        // label.enableStroke(cc.color(255, 255, 255, 255), 10);
    ...
    reset : function () {
        this.resetScreen();
        if(this.gameState == ACTIVITY_BUILD_SOMETHING_1.gameState.TEACHER_DEMO|| this.gameState == ACTIVITY_BUILD_SOMETHING_1.gameState.RESULT){
            this.emitSocketEvent(HDSocketEventType.GAME_MESSAGE, {
...
ACTIVITY_BUILD_SOMETHING_1.HDCardCell = cc.TableViewCell.extend({
    ...
    createUI: function (idx, data, parent, isSelected) {
        ...
        // this.cardElementImage.setContentSize(cc.size(colourLayer._contentSize.width, colourLayer._contentSize.height));
        ...
        // this.cardElementImage.setScaleX(0.5);
        // this.cardElementImage.setScaleY(0.5);
        ...
        // this.highlightLayer = new cc.LayerColor(cc.color(ACTIVITY_BUILD_SOMETHING_1.config.graphicalAssets.carouselBoxHighlight.color.r,ACTIVITY_BUILD_SOMETHING_1.config.graphicalAssets.carouselBoxHighlight.color.g,ACTIVITY_BUILD_SOMETHING_1.config.graphicalAssets.carouselBoxHighlight.color.b, ACTIVITY_BUILD_SOMETHING_1.config.graphicalAssets.carouselBoxHighlight.color.a), this._contentSize.width , this._contentSize.height);
...
ACTIVITY_BUILD_SOMETHING_1.ResultLayer = HDBaseLayer.extend({
    ...
    buttonCallback : function (pSender, type) {
        if (type ==ccui.Widget.TOUCH_ENDED ) {
            //console.log("pSender Tag",pSender.parent.getTag(), this.data[pSender.parent.getTag()], this.data);

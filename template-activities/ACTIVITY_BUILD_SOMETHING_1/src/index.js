...
ACTIVITY_BUILD_SOMETHING_1.BuildSomething = HDBaseLayer.extend({
  ...
  onEnter: function () {
    ...
    cc.loader.loadJson("res/Activity/" + activityName + "/config.json", function (error, config) {
      ...
      ACTIVITY_BUILD_SOMETHING_1.ref.dismantledObject = [
        ...ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.dismantledObject.data,
      ];
      ...
      ACTIVITY_BUILD_SOMETHING_1.config.teacherScripts.data.moduleStart.enable &&
        ACTIVITY_BUILD_SOMETHING_1.ref.triggerScript(
          ACTIVITY_BUILD_SOMETHING_1.config.teacherScripts.data.moduleStart.content.ops
        );
    });
  },
  ...
  background: function () {
    let bg = this.addSprite(
      ACTIVITY_BUILD_SOMETHING_1.spriteBasePath + "background.png",
  ...
  setupObjects: function () {
    ...
    for (var index = 0; index < dismantledObjects.length; index++) {
      ...
      var sprite = this.addSprite(
        ACTIVITY_BUILD_SOMETHING_1.spriteBasePath + currentObject.imageName,
        currentObject.initialPosition,
        this
      );
  ...
  setupCenterTable: function () {
    var centerLayer = this.createColourLayer(
      cc.color(100, 0, 0, 0),
      960,
      640,
      cc.p(cc.winSize.width * 0, cc.winSize.height * 0),
      this,
      1
    );
  ...
  setupOptionButtons: function () {
    if (ACTIVITY_BUILD_SOMETHING_1.ref.isTeacherView) {
      ...
      let startButton = this.createButton(
        ACTIVITY_BUILD_SOMETHING_1.spriteBasePath + startButtonObject.enableState,
        ACTIVITY_BUILD_SOMETHING_1.spriteBasePath + startButtonObject.pushedState,
        "",
        32,
        ACTIVITY_BUILD_SOMETHING_1.Tag.startButton,
        cc.p(this.getContentSize().width * 0.09, this.getContentSize().height * 0.2),
        this,
        this,
        ACTIVITY_BUILD_SOMETHING_1.spriteBasePath + startButtonObject.disableState
      );
      ...
      let stopButton = this.createButton(
        ACTIVITY_BUILD_SOMETHING_1.spriteBasePath + stopButtonObject.enableState,
        ACTIVITY_BUILD_SOMETHING_1.spriteBasePath + stopButtonObject.pushedState,
        "",
        32,
        ACTIVITY_BUILD_SOMETHING_1.Tag.stopButton,
        startButton.getPosition(),
        this,
        this,
        ACTIVITY_BUILD_SOMETHING_1.spriteBasePath + stopButtonObject.disableState
      );
  ...
  updateRoomData: function () {
    ...
    let data =
      ACTIVITY_BUILD_SOMETHING_1.ref.gameState == ACTIVITY_BUILD_SOMETHING_1.gameState.TEACHER_DEMO
        ? this.userDataForSync
        : this.allUserData;
    data =
      ACTIVITY_BUILD_SOMETHING_1.ref.gameState == ACTIVITY_BUILD_SOMETHING_1.gameState.RESULT
        ? this.resultScreenDataForSync
        : data;
  ...
  onMouseMove: function (event) {
    ...
    if (ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem && ACTIVITY_BUILD_SOMETHING_1.ref.mousePressedDown) {
      ...
      ACTIVITY_BUILD_SOMETHING_1.ref.moveObject(
        loc,
        ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.tag - ACTIVITY_BUILD_SOMETHING_1.Tag.movedObject,
        ACTIVITY_BUILD_SOMETHING_1.events.MOVE
      );
    } else {
  ...
  onMouseUp: function (event) {
    ...
    if (ACTIVITY_BUILD_SOMETHING_1.ref.tableView) {
      ...
      var loc = ACTIVITY_BUILD_SOMETHING_1.ref.convertToNodeSpace(event.getLocation());
      if (
        ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem &&
        cc.rectIntersectsRect(
          ACTIVITY_BUILD_SOMETHING_1.ref.tableView.getBoundingBox(),
          ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getBoundingBox()
        )
      ) {
        ACTIVITY_BUILD_SOMETHING_1.ref.moveBackChanges();

        ACTIVITY_BUILD_SOMETHING_1.ref.moveObject(
          ACTIVITY_BUILD_SOMETHING_1.ref.initialPosition,
          "",
          ACTIVITY_BUILD_SOMETHING_1.events.MOVE_BACK
        );
        return;
      } else {
        var finalPosition = ACTIVITY_BUILD_SOMETHING_1.ref.matchPosition(loc);
        ACTIVITY_BUILD_SOMETHING_1.ref.moveObject(
          finalPosition ? finalPosition : loc,
          "",
          ACTIVITY_BUILD_SOMETHING_1.events.STOP
        );
      }
    }
    ACTIVITY_BUILD_SOMETHING_1.ref.updateRoomData();
  ...
  moveObjectWithMouse: function (location) {
    if (!this.mousePressedDown) return;
    ...
    this.draggableObject.forEach((element) => {
      if (
        element &&
        element.parent &&
        element.cardElementImage.isVisible() &&
        cc.rectContainsPoint(element.cardElementImage.getBoundingBox(), element.convertToNodeSpace(location))
      ) {
  ...
  moveObject: function (position, index, event) {
    if (this.isTeacherView && this.gameState != ACTIVITY_BUILD_SOMETHING_1.gameState.TEACHER_DEMO) return;
  ...
  matchPosition: function (location) {
    ...
    if (this.clickedItem) {
      ...
      if (result.isIsolatedNode) {
        ...
        ACTIVITY_BUILD_SOMETHING_1.ref.moveObject(
          ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getPosition(),
          "",
          ACTIVITY_BUILD_SOMETHING_1.events.STOP
        );
        ACTIVITY_BUILD_SOMETHING_1.ref.validateData(
          ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.tag - ACTIVITY_BUILD_SOMETHING_1.Tag.movedObject,
          ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getPosition(),
          ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getContentSize(),
          true,
          null
        );
        ...
        if (
          ...
          clickedItem &&
          attachResult.snapIdxOfPlacedObj != -1 &&
          attachResult.snapIdxOfDraggedObj != -1
        ) {
          // Placed Obj
        ...
        let completeMovement = cc.callFunc(function () {
          if (ACTIVITY_BUILD_SOMETHING_1.ref.tableView) {
            ...
            ACTIVITY_BUILD_SOMETHING_1.ref.validateData(
              ...
              ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getContentSize(),
              false,
              result.connectionNodeName,
              attachResult.isAttachedToCorrectPoints
            );
            ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem = null;
            ACTIVITY_BUILD_SOMETHING_1.ref.updateTableView();
          }
        ...
        if (attachResult.attachTo) {
          ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.runAction(
            cc.sequence([cc.moveTo(0.1, attachResult.attachTo), completeMovement])
          );
        }
  ...
  updateUserDataForSync: function (nodeName, position, size, data, correct, isNew, index) {
    ...
    for (let object of ACTIVITY_BUILD_SOMETHING_1.config.gameInfo.assembleObjectInfo) {
      for (let assembledObj of this.userDataForSync) {
        if (
          assembledObj.isCorrect &&
          object.name == assembledObj.nodeName &&
          object.partsCount == assembledObj.data.length &&
          !assembledObj.isAnimationCompled
        ) {
          this.allPartsConnectedSuccessfully(assembledObj, object);
          assembledObj.isAnimationCompled = true;
          if (
            ACTIVITY_BUILD_SOMETHING_1.ref.isTeacherView &&
            ACTIVITY_BUILD_SOMETHING_1.ref.gameState == ACTIVITY_BUILD_SOMETHING_1.gameState.TEACHER_DEMO
          ) {
            this.emitSocketEvent(HDSocketEventType.GAME_MESSAGE, {
  ...
  moveBackChanges: function () {
    ...
    if (ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem) {
      ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.runAction(
        cc.sequence([cc.moveTo(0.25, this.initialPosition), completion])
      );
    }
  },
  ...
  attachPoint: function (loc, index) {
    ...
    var connectionObjectData =
      ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.dismantledObject.data[connectionObject.getUserData().tag];
    var clickedItemData =
      ACTIVITY_BUILD_SOMETHING_1.ref.dismantledObject[
        ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.tag - ACTIVITY_BUILD_SOMETHING_1.Tag.movedObject
      ];
    ...
    if (connectionObject && connectionObjectData && clickedItemData) {
      for (let index = 0; index < connectionObjectData.snappingPoints.length; index++) {
        var connectionSnapPoint = cc.p(
          connectionObjectData.snappingPoints[index].x * connectionObject.getScaleX(),
          connectionObjectData.snappingPoints[index].y * connectionObject.getScaleY()
        );
        var pointInWorldSpace = cc.p(
          connectionObject.getPositionX() -
            connectionObject.getContentSize().width * connectionObject.getScaleX() * 0.5 +
            connectionSnapPoint.x,
          connectionObject.getPositionY() -
            connectionObject.getContentSize().height * connectionObject.getScaleY() * 0.5 +
            connectionSnapPoint.y
        );
        for (let currIndex = 0; currIndex < clickedItemData.snappingPoints.length; currIndex++) {
          var currentSnapPoint = cc.p(
            clickedItemData.snappingPoints[currIndex].x * ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getScaleX(),
            clickedItemData.snappingPoints[currIndex].y * ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getScaleY()
          );
          var currentPointInWorldSpace = cc.p(
            ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getPositionX() -
              ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getContentSize().width *
                ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getScaleX() *
                0.5 +
              currentSnapPoint.x,
            ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getPositionY() -
              ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getContentSize().height *
                ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getScaleY() *
                0.5 +
              currentSnapPoint.y
          );
    ...
    isAttachedToCorrectPoints = this.checkIfAttachedToCorrectPoints(
      clickedItemData,
      originalPoint,
      attachedItemSnapPoint
    );
    if (attachToLocation && originalSnappingPoint) {
      var newAttachToLocation = cc.p(
        attachToLocation.x +
          ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getContentSize().width *
            ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getScaleX() *
            0.5 -
          originalSnappingPoint.x,
        attachToLocation.y +
          ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getContentSize().height *
            ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getScaleY() *
            0.5 -
          originalSnappingPoint.y
      );
      return {
  ...
  checkIfAttachedToCorrectPoints: function (clickedItemData, originalPoint, attachedItemSnapPoint) {
    let matched = false;
    if (!clickedItemData || !originalPoint || !attachedItemSnapPoint.snapDetails) return;
    if (
      clickedItemData.name == attachedItemSnapPoint.snapDetails.name &&
  ...
  playCharacterAnimation: function (animationInfo, position) {
    ...
    sprite.runAction(
      HDUtility.runFrameAnimation(
        ACTIVITY_BUILD_SOMETHING_1.animationBasePath + animationInfo.animationPath,
        16,
        0.05,
        ".png",
        true
  ...
  isTouchesPreviouslyPlacedObject: function (loc) {
    for (let index = 0; index < ACTIVITY_BUILD_SOMETHING_1.ref.assembledList.length; index++) {
      if (
        cc.rectIntersectsRect(
          ACTIVITY_BUILD_SOMETHING_1.ref.assembledList[index].getBoundingBox(),
          ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.getBoundingBox()
        )
      ) {
        return {
  ...
  socketListener: function (res) {
    if (!ACTIVITY_BUILD_SOMETHING_1.ref) return;
    switch (res.eventType) {
      ...
      case HDSocketEventType.GAME_MESSAGE:
        if (res.data.userName == HDAppManager.username) return;
    ...
    switch (data.event) {
      ...
      case ACTIVITY_BUILD_SOMETHING_1.events.STOP:
        if (ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem) {
            ...
            if (ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem) {
              if (data.position) {
                if (ACTIVITY_BUILD_SOMETHING_1.ref.gameState == ACTIVITY_BUILD_SOMETHING_1.gameState.TEACHER_DEMO) {
                  ACTIVITY_BUILD_SOMETHING_1.ref.assembledList.push(ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem);
                  ACTIVITY_BUILD_SOMETHING_1.ref.dismantledObject.splice(
                    ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.tag - ACTIVITY_BUILD_SOMETHING_1.Tag.movedObject,
                    1
                  );
                  ACTIVITY_BUILD_SOMETHING_1.ref.tableView.reloadData();
          ...
          ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.runAction(
            cc.sequence([cc.moveTo(0.25, data.position), removeClickedItem])
          );
        }
        break;
  ...
  resetScreen: function () {
    ...
    if (this.isResultViewOn) {
      ...
      if (this.isTeacherView) {
        ACTIVITY_BUILD_SOMETHING_1.ref.parent.updateScript(
          ACTIVITY_BUILD_SOMETHING_1.config.teacherScripts.data.moduleStart.content.ops
        );
      }
    }
    ...
    ACTIVITY_BUILD_SOMETHING_1.ref
      .getChildByTag(ACTIVITY_BUILD_SOMETHING_1.Tag.backgroundSprite)
      .setTexture(ACTIVITY_BUILD_SOMETHING_1.spriteBasePath + "background.png");
  },
      ...
      case ccui.Widget.TOUCH_ENDED:
        switch (sender.getTag()) {
          ...
          case ACTIVITY_BUILD_SOMETHING_1.Tag.leftButton:
            ...
            var offset = 60;
            var finalOffset = cc.clampf(
              this.tableView.getContentOffset().x + offset,
              this.tableView.minContainerOffset().x,
              this.tableView.maxContainerOffset().x
            );
  ...
  updateScrollOffset: function (isLeft) {
    ...
    this.interval = setInterval((isLeft) => {
      if (
        ACTIVITY_BUILD_SOMETHING_1.ref.scrollEvent &&
        ACTIVITY_BUILD_SOMETHING_1.ref.scrollEvent == cc.EventMouse.DOWN
      ) {
  ...
  mouseControlEnable: function (location) {
    for (let obj of this.handIconUI) {
      if (
        obj &&
        obj.getParent() &&
        cc.rectContainsPoint(obj.getBoundingBox(), obj.getParent().convertToNodeSpace(location)) &&
  ...
  setCardContent: function () {
    ...
    let baseColorLayer = this.createColourLayer(
      cc.color(
        ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.carouselBackground.color.r,
        ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.carouselBackground.color.g,
        ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.carouselBackground.color.b,
        ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.carouselBackground.color.a
      ),
      width,
      this.carouselHeight,
      position,
    ...
    let carouselBaseImage = this.addSprite(
      ACTIVITY_BUILD_SOMETHING_1.spriteBasePath +
        ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.carouselBackgroundImage.imageName,
      cc.p(position.x - 10, position.y),
      this
    );
    ...
    let carouselBorderImage = this.addSprite(
      ACTIVITY_BUILD_SOMETHING_1.spriteBasePath +
        ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.carouselBackgroundBorderImage.imageName,
      cc.p(position.x - 10, position.y),
      this
    );
    ...
    let leftButton = this.createButton(
      ACTIVITY_BUILD_SOMETHING_1.spriteBasePath +
        ACTIVITY_BUILD_SOMETHING_1.config.buttons.data.leftMoveButton.enableState,
      ACTIVITY_BUILD_SOMETHING_1.spriteBasePath +
        ACTIVITY_BUILD_SOMETHING_1.config.buttons.data.leftMoveButton.pushedState,
      "",
      8,
      ACTIVITY_BUILD_SOMETHING_1.Tag.leftButton,
    ...
    leftButton.setPosition(
      cc.p(leftButton.getContentSize().width * 0.5, leftButton.getContentSize().height * 0.5 + 14)
    );
    leftButton.setSwallowTouches(false);

    let rightButton = this.createButton(
      ACTIVITY_BUILD_SOMETHING_1.spriteBasePath +
        ACTIVITY_BUILD_SOMETHING_1.config.buttons.data.rightMoveButton.enableState,
      ACTIVITY_BUILD_SOMETHING_1.spriteBasePath +
        ACTIVITY_BUILD_SOMETHING_1.config.buttons.data.rightMoveButton.pushedState,
      "",
      8,
      ACTIVITY_BUILD_SOMETHING_1.Tag.rightButton,
      cc.p(
        leftButton.getPositionX() + (width - leftButton.getContentSize().width),
        leftButton.getContentSize().height * 0.5 + 14
      ),
      baseColorLayer,
      null,
      ACTIVITY_BUILD_SOMETHING_1.spriteBasePath +
    ...
    this.tableView = new cc.TableView(
      this,
      cc.size(width - leftButton.getContentSize().width * 2, this.carouselHeight + 11)
    );
  ...
  getWidthOfCarousel: function () {
    ...
    for (let idx = 0; idx < ACTIVITY_BUILD_SOMETHING_1.ref.dismantledObject.length; ++idx) {
      var spriteTexture = new cc.Sprite(
        ACTIVITY_BUILD_SOMETHING_1.spriteBasePath + ACTIVITY_BUILD_SOMETHING_1.ref.dismantledObject[idx].imageName
      );
      let aspectRatio = spriteTexture.getContentSize().width / spriteTexture.getContentSize().height;
      var width =
        ACTIVITY_BUILD_SOMETHING_1.ref.carouselHeight * aspectRatio
          ? ACTIVITY_BUILD_SOMETHING_1.ref.carouselHeight * aspectRatio
          : ACTIVITY_BUILD_SOMETHING_1.ref.carouselHeight;
  ...
  isCellSelected: function (studentDetails) {
    ...
    this.cardQueue.forEach((studentName) => {
      if (studentName == studentDetails.name) status = true;
    });
    return status;
  },
  ...
  tableCellSizeForIndex: function (table, idx) {
    ...
    let aspectRatio = spriteTexture.getContentSize().width / spriteTexture.getContentSize().height;
    var width =
      table.getViewSize().height * aspectRatio ? table.getViewSize().height * aspectRatio : table.getViewSize().height;
  ...
  tableCellAtIndex: function (table, idx) {
    ...
    cardCell.createUI(
      idx,
      ACTIVITY_BUILD_SOMETHING_1.ref.dismantledObject[idx],
      cardCell,
      this.isCellSelected(ACTIVITY_BUILD_SOMETHING_1.ref.dismantledObject[idx])
    );
  ...
  moveCellObject: function (cell, position) {
    ...
    let completedAnimation = cc.callFunc(function () {
      if (ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem) return;

      var lastIndex =
        ACTIVITY_BUILD_SOMETHING_1.ref.cardQueue.length == 0
          ? -1
          : ACTIVITY_BUILD_SOMETHING_1.ref.getIndexOfCurrentElement(ACTIVITY_BUILD_SOMETHING_1.ref.cardQueue[0]);
      if (lastIndex != -1) {
      ...
      ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem = ACTIVITY_BUILD_SOMETHING_1.ref.addSprite(
        ACTIVITY_BUILD_SOMETHING_1.spriteBasePath + ACTIVITY_BUILD_SOMETHING_1.ref.dismantledObject[cell.tag].imageName,
        finalPosition,
        ACTIVITY_BUILD_SOMETHING_1.ref
      );
      ...
      ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.setName(
        ACTIVITY_BUILD_SOMETHING_1.ref.dismantledObject[cell.tag].name
      );
      ...
      ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.setLocalZOrder(
        ACTIVITY_BUILD_SOMETHING_1.ref.dismantledObject[cell.tag].localZOrder
      );
      ...
      if (ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem && ACTIVITY_BUILD_SOMETHING_1.ref.isTeacherView) {
        ACTIVITY_BUILD_SOMETHING_1.ref.moveObject(
          finalPosition,
          ACTIVITY_BUILD_SOMETHING_1.ref.clickedItem.tag - ACTIVITY_BUILD_SOMETHING_1.Tag.movedObject,
          ACTIVITY_BUILD_SOMETHING_1.events.START
        );
      }
    });
  ...
  showPreviewToAll: function (res) {
    ...
    let filteredArray = res.data.filter(
      (item, idx) => res.data.findIndex((obj, pos) => obj.nodeName === item.nodeName) === idx
    );
    ...
    if (
      filteredArray.length === ACTIVITY_BUILD_SOMETHING_1.config.gameInfo.assembleObjectInfo.length &&
      filteredArray.every(
        (item) =>
          item.isCorrect &&
          item.data.length ===
            ACTIVITY_BUILD_SOMETHING_1.config.gameInfo.assembleObjectInfo.find((obj) => obj.name === item.nodeName)
              .partsCount
      )
    ) {
      isAllCorrect = true;
    ...
    ACTIVITY_BUILD_SOMETHING_1.config.teacherScripts.data[
      isAllCorrect ? "TargetAssembledSuccessfully" : "TargetAssembledUnsuccessfully"
    ].enable &&
      this.triggerScript(
        ACTIVITY_BUILD_SOMETHING_1.config.teacherScripts.data[
          isAllCorrect ? "TargetAssembledSuccessfully" : "TargetAssembledUnsuccessfully"
    ...
    if (bg) {
      ...
      bg.setTexture(
        ACTIVITY_BUILD_SOMETHING_1.spriteBasePath +
          (isAllCorrect
            ? ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.winBG.imageName
            : ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.looseBG.imageName)
      );
    } else {
      bg = this.addSprite(
        ACTIVITY_BUILD_SOMETHING_1.spriteBasePath +
    ...
    var position = isAllCorrect
      ? ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.winBG.usernamePosition
      : ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.looseBG.usernamePosition;
    this.createUIForData(res.userName, res.data, bg);
    var touchPrevention = this.createButton(
      ACTIVITY_BUILD_SOMETHING_1.spriteBasePath + "close.png",
      null,
      null,
      null,
      null,
      cc.p(bg.width * 0.5, bg.height * 0.5),
      bg,
      this,
      null
    );
    ...
    if (ACTIVITY_BUILD_SOMETHING_1.ref.isTeacherView) {
      var cross = this.createButton(
        ACTIVITY_BUILD_SOMETHING_1.spriteBasePath + "close.png",
        null,
        null,
        null,
        ACTIVITY_BUILD_SOMETHING_1.Tag.closePreview,
        cc.p(bg.width * 0.9, bg.height * 0.85),
        bg,
        this,
        null
      );
...
ACTIVITY_BUILD_SOMETHING_1.HDCardCell = cc.TableViewCell.extend({
  ...
  createUI: function (idx, data, parent, isSelected) {
    ...
    let colourLayer = new cc.LayerColor(
      cc.color(
        ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.carouselBoxBackground.color.r,
        ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.carouselBoxBackground.color.g,
        ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.carouselBoxBackground.color.b,
        ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.carouselBoxBackground.color.a
      ),
      this.width - this.cellHorizontalPadding,
      this.height - this.cellVerticalPadding
    );
    ...
    var scaleFactorX =
      (colourLayer.getContentSize().height * 0.9 * aspectRatio) / this.cardElementImage.getContentSize().width;
    ...
    let textBaseLayer = new cc.LayerColor(
      cc.color(
        ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.carouselBoxCardBackground.color.r,
        ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.carouselBoxCardBackground.color.g,
        ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.carouselBoxCardBackground.color.b,
        ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.carouselBoxCardBackground.color.a
      ),
      this._contentSize.width - this.cellHorizontalPadding,
      this.cardTextHeight
    );
    textBaseLayer.setPosition(
      cc.p(this.cellHorizontalPadding * 0.5, this._contentSize.height - this.cellVerticalPadding - this.cardTextHeight)
    );
    parent.addChild(textBaseLayer, 7);

    let labelCardText = cc.LabelTTF.create(
      data.cardValue,
      ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.carouselBoxCardText.font,
      ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.carouselBoxCardText.fontSize,
      cc.size(0, 0),
      cc.TEXT_ALIGNMENT_CENTER
    );
    labelCardText.setPosition(cc.p(textBaseLayer._contentSize.width * 0.5, textBaseLayer._contentSize.height * 0.5));
    labelCardText.setColor(
      cc.color(
        ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.carouselBoxCardText.color.r,
        ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.carouselBoxCardText.color.g,
        ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.carouselBoxCardText.color.b,
        ACTIVITY_BUILD_SOMETHING_1.config.assets.sections.carouselBoxCardText.color.a
      )
    );
...
ACTIVITY_BUILD_SOMETHING_1.ResultLayer = HDBaseLayer.extend({
  ...
  setUpScrollContainer: function () {
    ...
    var innerContentHeight =
      this.totalRow <= 2
        ? this.height * 0.8
        : this.totalRow * this.width * this.getScaleFactor() + this.totalRow * verticalSpacing;
  ...
  populateDataInContainer: function () {
    ...
    var verticalSpacing = this.height * 0.07;
    var columnNo =
      this.data.length <= 2 ? this.data.length : this.data.length > 8 ? 4 : Math.ceil(this.data.length / 2);
    for (var index = 0; index < this.data.length; index++) {
      var frame = this.createFrame();
      var cell = this.createBanner();
      ACTIVITY_BUILD_SOMETHING_1.ref.createUIForData(
        ACTIVITY_BUILD_SOMETHING_1.ref.allUserData[index].userName,
        ACTIVITY_BUILD_SOMETHING_1.ref.allUserData[index].data,
        cell
      );
      ...
      frame.setTag(index);
      frame.setPosition(
        (index % columnNo) * (horizontalSpacing + this.getFramesWidth()),
        this.container.height -
          Math.floor((index + columnNo) / columnNo) * (frame.width * frame.getScale() + verticalSpacing)
      );
  ...
  createFrame: function () {
    ...
    var button = new ccui.Button(
      ACTIVITY_BUILD_SOMETHING_1.spriteBasePath + "reward_screen_frame.png",
      ACTIVITY_BUILD_SOMETHING_1.spriteBasePath + "reward_screen_frame.png",
      ACTIVITY_BUILD_SOMETHING_1.spriteBasePath + "reward_screen_frame.png"
    );
  ...
  addStudentName: function (name, parent) {
    var label = this.createTTFLabel(
      ...
      50,
      HDConstants.Black,
      cc.p(parent.width * 0.5, parent.height * 0.1),
      parent
    );
  },

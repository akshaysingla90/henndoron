...
ACTIVITY_KEVINS_CRANE_1.socketEventKey = {
    HELP                        : "KC101",
    CRANE_MOVEMENT              : "KC102",
    MOVE_WREACKING_BALL         : "KC103",
    CHANGE_MULTIPLAYER_MODE     : "KC104",
    CRANE_STOP                  : "KC105",
    REPLAY                      : "KC107",
    USER_DATA                   : "KC109",
    START_NEXT_LEVEL            : "KC110",
    BRICK_BROCK                 : "KC112",
    ANIMATION_ENDED_ACK         : "KC113",
    CHARACTER_PICKED            : "KC114",
    CHARACTER_PLACED            : "KC115",
    CHARACTER_DROPPED           : "KC116",
};
 ...
 *
 * Methods present :  moveDown(), moveUp(), addCollider() allCrane related animatios
 */
ACTIVITY_KEVINS_CRANE_1.CraneNode = cc.Node.extend({
    ...
    addColliderImage : function(characterName, opacity) {
        ...
        // console.log("character name", this.craneCockpitCollider.text);
    ...
    moveUp : function(data){
        var boundary = data ? data.cockpitPosition.y : this.craneCockpit.getPosition().y;
       if (boundary <= this.gameRef._size.height  * 0.85){
           if(!data){
               ...
               data.cockpitPosition.y = data.cockpitPosition.y + 1;
               data.colliderPosition.y = data.colliderPosition.y + 1;
               data.cylenderScaleY =   data.cylenderScaleY + 0.035
           }
        }
    },
    moveDown : function(data){
        var boundary = data ? data.cockpitPosition.y : this.craneCockpit.getPosition().y;
        if (boundary >= this.initialCockpitPos.y){
            if(!data){
                this.craneCockpit.setPosition(cc.p(this.craneCockpit.getPosition().x,this.craneCockpit.getPosition().y - 1));
                this.craneCockpitCollider.setPosition(cc.p(this.craneCockpitCollider.getPosition().x, this.craneCockpitCollider.getPosition().y - 1));
                this.craneCylinder.setScaleY(this.craneCylinder.getScaleY() - 0.035);
    ...
    resetCockPitImage : function () {
        ACTIVITY_KEVINS_CRANE_1.config.assets.sections.crane.data;
        var imageName =  ACTIVITY_KEVINS_CRANE_1.config.assets.sections.crane.data.craneCockpit.imageName;
        if (this.gameRef.gameType == ACTIVITY_KEVINS_CRANE_1.GAME_TYPE.CONSTRUCTION)
            imageName =  ACTIVITY_KEVINS_CRANE_1.config.assets.sections.crane.data.craneClaw.imageName;
        this.craneCockpit.setTexture(ACTIVITY_KEVINS_CRANE_1.spriteBasePath + imageName)
    }
});
 ...
 *
 * methods present : movecrane, touch listeners, shaftbuttoncallBack,  handel joystick information
 */
ACTIVITY_KEVINS_CRANE_1.CraneControl = cc.Node.extend({
    ...
    update : function(dt){
        ...
        this.moveCrane(dt);
        this.craneUpDown(dt);
        ACTIVITY_KEVINS_CRANE_1.ref.updateStudentCraneInfo();
    ...
    buttonCallback : function (sender, type){
        if(!ACTIVITY_KEVINS_CRANE_1.ref.isStudentInteractionEnable || ACTIVITY_KEVINS_CRANE_1.ref.isPreviewMode || ( ACTIVITY_KEVINS_CRANE_1.ref.isTeacherView && ACTIVITY_KEVINS_CRANE_1.ref.isStudentSelected && ACTIVITY_KEVINS_CRANE_1.ref.isTurnBased())) return;
        var button      = sender;
        var Tag   = button.tag ;
        switch (type) {
    ...
    changeTouchResponsiveness : function(ref, isTouchEnabled) {
        if(!ACTIVITY_KEVINS_CRANE_1.ref.isStudentInteractionEnable || ACTIVITY_KEVINS_CRANE_1.ref.isStudentSelected)  return;
        console.log("change touch responsiveness");
    ...
    setShaftFrame : function(angle, isIdle){
        this.joystickDirection = this.getJoyStickDirection(angle);
        // console.log("set direaction", this.joystickDirection )
        // console.trace();
        if (isIdle){
    ...
    getJoyStickDirection : function(angle){
        if(angle == 0){
           return 0;
        }
        if (angle >= 125 && angle <= 200){
    ...
    onTouchMoved:function(touch, event) {
        if(!ACTIVITY_KEVINS_CRANE_1.ref.isStudentInteractionEnable || ACTIVITY_KEVINS_CRANE_1.ref.isPreviewMode || (ACTIVITY_KEVINS_CRANE_1.ref.isStudentSelected && ACTIVITY_KEVINS_CRANE_1.ref.isTurnBased())) return;
        var pos = touch.getLocation();
        if (this.shaftTouch(pos)){
            let newPoints = this.nobShaft.parent.convertToNodeSpace(pos);
            let angle = Math.atan2(newPoints.y - this.nobShaft.getPosition().y, newPoints.x - this.nobShaft.getPosition().x) * 180 / Math.PI + 180;
    ...
    onTouchEnded:function(touch, event) {
        if(!ACTIVITY_KEVINS_CRANE_1.ref.isStudentInteractionEnable || ACTIVITY_KEVINS_CRANE_1.ref.isPreviewMode || (ACTIVITY_KEVINS_CRANE_1.ref.isStudentSelected && ACTIVITY_KEVINS_CRANE_1.ref.isTurnBased() )|| this.isAnimationStarted) return;
        this.craneNode.stopMoveAnimation();
        this.angle = 0;
    ...
    sendCraneFinalPositionToOthers(angle, isIdle){
        var dataToSend = ACTIVITY_KEVINS_CRANE_1.ref.createDataToSend();  // sending final position
        ACTIVITY_KEVINS_CRANE_1.ref.updateUserInfo(dataToSend);
           ACTIVITY_KEVINS_CRANE_1.ref.emitSocketEvent(HDSocketEventType.GAME_MESSAGE, {
...
ACTIVITY_KEVINS_CRANE_1.KevinsCrane = HDBaseLayer.extend({
    ...
    onEnter: function () {
        ...
        cc.loader.loadJson("res/Activity/" + activityName + "/config.json", function (error, config) {
            ...
            console.log("config",ACTIVITY_KEVINS_CRANE_1.config );
            ...
            if (ACTIVITY_KEVINS_CRANE_1.ref.isTeacherView) {
                ACTIVITY_KEVINS_CRANE_1.ref.createDataToSend(); // teacher view on Enter
    ...
    onExit: function () {
        ...
        ACTIVITY_KEVINS_CRANE_1.ref.craneInfo           =  null;
        console.log("on Exit",ACTIVITY_KEVINS_CRANE_1.ref.syncInfo,  ACTIVITY_KEVINS_CRANE_1.ref.gamePlayInfo,  ACTIVITY_KEVINS_CRANE_1.ref.gameData);
        ACTIVITY_KEVINS_CRANE_1.ref                     =   null;
    },
    ...
    //     //         console.log(err, res);
    //     //        if(res && res.data.data.activity == ACTIVITY_KEVINS_CRANE_1.config.namespace) {
    //     //            ACTIVITY_KEVINS_CRANE_1.ref.syncInfo = res.data.data.data;
    //     //            console.log("syncInfo", ACTIVITY_KEVINS_CRANE_1.ref.syncInfo);
    //     //            ACTIVITY_KEVINS_CRANE_1.ref.updateUI();
    //     //        }else{
    ...
    setupUI: function () {
        ...
        if(this.isTeacherView){
            let multiPlayerMode = this.createButton(ACTIVITY_KEVINS_CRANE_1.spriteBasePath + ACTIVITY_KEVINS_CRANE_1.config.buttons.data.multiplayerMode.enableState,
                null, "Go to Individual Mode", 12, ACTIVITY_KEVINS_CRANE_1.Tag.MULTIPLAYER_MODE, cc.p(this.getContentSize().width * 0.1, this.getContentSize().height * 0.15), this);
        ...
        this.buildHome();
        console.log("game Data", this.gameData.length);
        this.addCraneInGame();
        ...
        if(this.isTurnBased() && this.isTeacherView){
        ...
        var dataToSend = this.createDataToSend();       // user data set up UI
    ...
    buildHome: function() {
        ...
        for (var floorBalancerCounter   = this.maxNumberOfRows - 1; floorBalancerCounter > 0; floorBalancerCounter--) {
            var floorBalancer           =   this.addSprite(ACTIVITY_KEVINS_CRANE_1.spriteBasePath + ACTIVITY_KEVINS_CRANE_1.config.assets.sections.building.data[balancerIndex].imageName,
    ...
    addDustImage : function() {
        ...
        dustImage.setTag(ACTIVITY_KEVINS_CRANE_1.Tag.DUST_IMAGE)
    ...
    removeDustImage: function(){
        if(this.getChildByTag(ACTIVITY_KEVINS_CRANE_1.Tag.DUST_IMAGE)){
            this.removeChildByTag(ACTIVITY_KEVINS_CRANE_1.Tag.DUST_IMAGE)
        };
    },
    ...
    addCharacterInGrounds : function() {
        ...
        this.maxCharacterInGround = this.groundLetters.length;
    ...
    characterPickingProcess : function() {
        ...
        if(this.isStudentInteractionEnable && ((this.isTurnBased() && !this.isStudentSelected) ||(!this.isTurnBased() && !this.isPreviewMode)) ){
            var dataToSend = this.createDataToSend("characterPickingProcess");
            dataToSend.userGamePlay.userData.craneInfo.colliderPosition = colliderPosition;
    ...
    elementPlacedSuccessfully(data){
        ...
        this.addNewCharacter(this,data);
        this.runAction(cc.sequence(cc.callFunc(this.craneControlNode.craneNode.runCraneElementPlaceAnimation, this.craneControlNode.craneNode), cc.delayTime(0.5), cc.callFunc(this.removeGroundCharacterRef, this, data), cc.callFunc(this.updateGameRowData, this, data)));
        this.craneControlNode.craneNode.addColliderImage(" ", 0);
    },
    elementDropped(data){
        this.craneControlNode.craneNode.addColliderImage(" ", 0);
        var fallAnimation = this.fallCharacterToGround(data);
        this.craneControlNode.craneNode.runCraneElementPlaceAnimation();
        this.selectedCharacterFromGround.runAction(cc.sequence(cc.fadeIn(0), cc.delayTime(0.5), fallAnimation ));
    },
    ...
    addAllImagesInBoard : function() {
        ...
        this.hintImagesBase.setTag(ACTIVITY_KEVINS_CRANE_1.Tag.HELP_IMAGE_TAG);
    ...
    showResetHover() {
        console.log("show reset hover", this.isResetAnimationPlaying);
        ...
        this.resetHoverImage.runAction(cc.sequence(cc.moveBy(0.3, movement, 0), cc.delayTime(1), cc.callFunc(this.setupCharacters,this), cc.delayTime(1.0), cc.callFunc(this.reconstructHome,this), cc.moveBy(0.3, -movement, 0),cc.callFunc(this.sendAnimationEndAck, this), cc.delayTime(3), cc.callFunc(this.enableControllerTouch, this)));
    },
    ...
    fallCharacterToGround : function(data) {
        ...
        if((this.isStudentInteractionEnable && !this.isStudentSelected && this.isTurnBased()) || !this.isTurnBased() && !this.isPreviewMode){
            ...
            var dataToSend = this.createDataToSend();
            this.updateUserInfo(dataToSend, "fallCharacterToGround");
    ...
    updateGameRowData(ref, rowData) {
       ...
       * */
        ...
        if((this.isStudentInteractionEnable && !this.isStudentSelected && this.isTurnBased()) || (!this.isTurnBased() && !this.isPreviewMode)) {
            ...
            if(this.gameType == ACTIVITY_KEVINS_CRANE_1.GAME_TYPE.CONSTRUCTION){
                if (this.checkIfWordConstructionPossible(rowData.rowCounter)) {
    ...
    playWordSuccessAnimation : function(){
        ...
        console.log(this.checkIfAllWordsCompleted() && ((this.isTurnBased() && !this.isStudentSelected) || (!this.isTurnBased() && !this.isPreviewMode)));
    ...
    moveToNextLevel : function() {
        console.log("move to next round cllaed");
    ...
    sendAnimationEndAck : function(){
        // console.log("reset send ack to all");
    ...
    onUpdateStudentInteraction: function (params) {
        console.log("update student intraction" , params);
        if (params.userName == HDAppManager.username) {
            ACTIVITY_KEVINS_CRANE_1.ref.isStudentInteractionEnable = params.status;
            ACTIVITY_KEVINS_CRANE_1.ref.craneControlNode.shaftButton.setTouchEnabled( params.status);
        }
    ...
    socketListener: function (res) {
        ...
        switch (res.eventType) {
            ...
            case HDSocketEventType.GAME_MESSAGE:
                ...
                break;
            case HDSocketEventType.SOCKET_DISCONNECT:
                if(!ACTIVITY_KEVINS_CRANE_1.ref.isTeacherView){
                    ACTIVITY_KEVINS_CRANE_1.ref.disableInteraction(false);
                    ACTIVITY_KEVINS_CRANE_1.ref.craneControlNode.shaftButton.setTouchEnabled(false);
                }
    ...
    studentTurn: function (res) {
        ...
        if(!this.isTeacherView && this.isStudentInteractionEnable){
            ACTIVITY_KEVINS_CRANE_1.ref.craneControlNode.onTouchEnded(null);

        }
        console.log("studentTurn", users);
        if (this.isTeacherView) {
            this.isStudentSelected = users.length > 0 ? true : false;
            if(this.isStudentSelected){
                console.log("student selected right now");
                this.craneControlNode.shaftButton.setTouchEnabled(false);
            }else{
                console.log(" no student selected right now");
    ...
    studentStatus: function (data) {
        ...
        // check if newly added and not treacher
    ...
    gameEvents: function (res) {
        ...
        switch (res.eventType) {
            ...
            case ACTIVITY_KEVINS_CRANE_1.socketEventKey.CHANGE_MULTIPLAYER_MODE:
                ...
                console.log("this.multiPlayerType", type);
                break;
    ...
    updateCranePosition : function(data){
        console.log("update crane position", data);
        ACTIVITY_KEVINS_CRANE_1.ref.craneControlNode.setShaftFrame(data.angle, data.isIdle);
    },
    stopCrane : function(data){
        // console.log("stop crane", data);
        ACTIVITY_KEVINS_CRANE_1.ref.craneControlNode.setShaftFrame(0, data.isIdle);
        if(!data.isIdle){
            this.craneControlNode.craneNode.startMoveAnimation();
        }
        this.craneControlNode.previousDirection = ACTIVITY_KEVINS_CRANE_1.ref.craneControlNode.joystickDirection;
    ...
    updateUserInfo : function(data, caller) {
       ...
       for (let user in this.gamePlayInfo) {
                if (user == data.userGamePlay.username) {
                    ...
                    // this.updateInfoForGamePlayInfo(data.userGamePlay.userData);
                    userExist = true;
                    break;
                }
        }
    ...
    updateUI : function(){
        ...
        var type =  ACTIVITY_KEVINS_CRANE_1.ref.multiPlayerType == 1 ?"individual" : "turnbased";
        console.log("multiplayer type", type);
        ...
        if(this.multiPlayerType == ACTIVITY_KEVINS_CRANE_1.MULTIPLAYER_TYPE.INDIVIDUAL){
            for(let user in this.gamePlayInfo) {
                if (user == HDAppManager.username) {
                    ...
                    this.updateRowAndGroundElement(user);
    ...
    updateRowAndGroundElement : function(user){
        let index =0;
        for(let item of this.gamePlayInfo[user].rowInfo){
            this.gameData[index++]           = {...item};
        }
        index=0;
        if(this.gamePlayInfo[user].groundCharacter.length >0){
            for(let item of this.gamePlayInfo[user].groundCharacter){
                this.groundElements[index++]      = {...item};
            }
        }
    },
    ...
    showPreview : function(userName){
        ...
        console.log("previewing student name", previewingStudent);
        ACTIVITY_KEVINS_CRANE_1.ref.stopAllActions();
        ACTIVITY_KEVINS_CRANE_1.ref.craneControlNode.setShaftFrame(0, true);
        this.craneControlNode.craneNode.stopMoveAnimation();
        this.craneControlNode.craneNode.craneCylinder.stopAllActions();
        this.craneControlNode.craneNode.craneCockpit.stopAllActions();
        this.craneControlNode.craneNode.craneCockpitCollider.stopAllActions();
        this.craneControlNode.craneNode.resetCockPitImage();
        this.resetHoverImage.setPositionX(-this.getContentSize().width * 0.1);
        if(this.selectedCharacterFromGround){
            this.selectedCharacterFromGround.stopAllActions();
        }
        if(this.gamePlayInfo[previewingStudent]){
            this.updateRowAndGroundElement(previewingStudent);
            ...
            if (this.gamePlayInfo[previewingStudent].characterPicked) {
                ...
                this.craneControlNode.craneNode.addColliderImage(ACTIVITY_KEVINS_CRANE_1.ref.selectedCharacterFromGround.text, 255);
                ACTIVITY_KEVINS_CRANE_1.ref.selectedCharacterFromGround.runAction(cc.sequence(cc.rotateTo(0, 0), cc.moveTo(0, this.craneControlNode.craneNode.getPosition().x + this.craneControlNode.craneNode.craneCockpitCollider.getPosition().x, ACTIVITY_KEVINS_CRANE_1.ref.selectedCharacterFromGround.getPositionY()), cc.fadeOut(0)));
            }else {
                this.craneControlNode.craneNode.addColliderImage(" ", 0);
            }
    ...
    syncCranePosition(data){
        ...
        ACTIVITY_KEVINS_CRANE_1.ref.craneControlNode.setShaftFrame(data.angle, data.angle ==0 ? true : false);
       if(data.angle ==0){
           this.craneControlNode.craneNode.stopMoveAnimation();
       }
    },
    updateHintAndHelpImage(){
        this.removeChildByTag(ACTIVITY_KEVINS_CRANE_1.Tag.HINT_BASE);
        this.removeChildByTag(ACTIVITY_KEVINS_CRANE_1.Tag.HELP_IMAGE_TAG);
        this.addHintImagesBase();
    ...
    restart : function(){
        ...
        if(this.isTeacherView) {
            ...
            // console.log("childCount",container.getChildrenCount());
    ...
    createDataToSend : function(){
        ...
        var data  = {
            ...
            "groundCharacter"   : groundElements,
        }
            ...
            case ccui.Widget.TOUCH_ENDED:
                switch (sender.getTag()) {
                    ...
                    case ACTIVITY_KEVINS_CRANE_1.Tag.MULTIPLAYER_MODE:
                        ...
                        ACTIVITY_KEVINS_CRANE_1.ref.multiPlayerType = ACTIVITY_KEVINS_CRANE_1.MULTIPLAYER_TYPE.INDIVIDUAL;
                        var type =  ACTIVITY_KEVINS_CRANE_1.ref.multiPlayerType == 1 ?"individual" : "turnbased";
                        console.log("multiplayer type", type );
                        lesson_1.ref.turnBased = false;
                        lesson_1.ref.setStudentsPreviewPanelActive(true);
    ...
    updateMouseIcon: function (location) {
        ...
        // console.log("hand icone",handICon);
        if(handICon){
            ...
            if (location.y < this.getContentSize().height * 0.9 && ACTIVITY_KEVINS_CRANE_1.ref.isStudentInteractionEnable && cc.rectContainsPoint(this.craneControlNode.controlBase.getBoundingBoxToWorld(),location)) {
                // console.log("inside change cursor image in update game datat");
                this.changeMouseCursorImage();
                this.customTexture = ACTIVITY_KEVINS_CRANE_1.ref.isStudentInteractionEnable;
                this.interactableObject = true;
            } else{
                this.customTexture = false;

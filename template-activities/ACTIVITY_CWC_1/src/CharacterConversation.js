...
ACTIVITY_CWC_1.CharacterConversationLayer = HDBaseLayer.extend({
    ...
    characterSetup: function () {
        //character 1 setup
        if (ACTIVITY_CWC_1.config.charactersData[0]) {
            let character_1 = this.addSprite(ACTIVITY_CWC_1.spriteBasePath + ACTIVITY_CWC_1.config.charactersData[0].imageName, cc.p(ACTIVITY_CWC_1.config.charactersData[0].position.x, ACTIVITY_CWC_1.config.charactersData[0].position.y), this);
            character_1.setTag(ACTIVITY_CWC_1.Tag.character_1);
            character_1.setScale(0.6);
            this.playAnimation(ACTIVITY_CWC_1.Tag.character_1, 0);
            if (this.isTeacherView) {
                this.textPanelSetup(character_1, 0, cc.p(character_1.getPositionX() - character_1.getContentSize().width * 0.135, character_1.getPositionY() - character_1.getContentSize().height * 0.28 - this._size.height * 0.26),
                    ACTIVITY_CWC_1.Tag.character_1_tableView, ACTIVITY_CWC_1.Tag.character_1_textStart, ACTIVITY_CWC_1.Tag.character_1_selectBtn,this.playSound);
            }
        }
        //character 2 setup;
        if (ACTIVITY_CWC_1.config.charactersData[1]) {
            let character_2 = this.addSprite(ACTIVITY_CWC_1.spriteBasePath + ACTIVITY_CWC_1.config.charactersData[1].imageName, cc.p(ACTIVITY_CWC_1.config.charactersData[1].position.x, ACTIVITY_CWC_1.config.charactersData[1].position.y), this);
            character_2.setTag(ACTIVITY_CWC_1.Tag.character_2);
            character_2.setScale(0.6);
            this.playAnimation(ACTIVITY_CWC_1.Tag.character_2, 1);
            if (this.isTeacherView) {
                this.textPanelSetup(character_2, 1, cc.p(character_2.getPositionX() - character_2.getContentSize().width * 0.135, character_2.getPositionY() - character_2.getContentSize().height * 0.28 - this._size.height * 0.26),
                    ACTIVITY_CWC_1.Tag.character_2_tableView, ACTIVITY_CWC_1.Tag.character_2_textStart,ACTIVITY_CWC_1.Tag.character_2_selectBtn ,this.playSound);
            }
        }
        //   character_ 3 Setup
        if (ACTIVITY_CWC_1.config.charactersData[2]) {
            let character_3 = this.addSprite(ACTIVITY_CWC_1.spriteBasePath + ACTIVITY_CWC_1.config.charactersData[2].imageName, cc.p(ACTIVITY_CWC_1.config.charactersData[2].position.x, ACTIVITY_CWC_1.config.charactersData[2].position.y), this);
            character_3.setTag(ACTIVITY_CWC_1.Tag.character_3);
            character_3.setScale(0.6);
            this.playAnimation(ACTIVITY_CWC_1.Tag.character_3, 2);
            if (this.isTeacherView) {
                this.textPanelSetup(character_3, 0, cc.p(character_3.getPositionX() - character_3.getContentSize().width * 0.135, character_3.getPositionY() - character_3.getContentSize().height * 0.28 - this._size.height * 0.26),
                    ACTIVITY_CWC_1.Tag.character_3_tableView, ACTIVITY_CWC_1.Tag.character_3_textStart,ACTIVITY_CWC_1.Tag.character_3_selectBtn ,this.playSound);
            }
        }
    },
    ...
    tableCellTouched: function (table, cell) {
        // cell.stopAllActions();
        // cell.runAction( cc.sequence(cc.scaleTo(0.1, 1.02, 1.02), cc.scaleTo(0.2,1, 1)));
        // this.playSound(cell);
    ...
    textPanelSetup : function(parent, index, position, scrollViewTag, startTag, selectButtonTag, callback){
      ...
      //  console.log(" position.y + sprite.getContentSize().height ", position.y + sprite.getContentSize().height,  sprite.getContentSize().height, position.y);
     ...
     //   console.log("sprite size", sprite.getContentSize());
    ...
    playSound : function(cell) {
        if(cell && cell.getParent() && cell.getParent().isVisible()) {
            ...
            cc.audioEngine.playEffect(ACTIVITY_CWC_1.soundPath + ACTIVITY_CWC_1.config.charactersData[key].audioData[index].audio);
            if(!this.soundPlayTriggered) {
                ...
                if(allSoundPlayed && !this.soundPlayTriggered) {
                    console.log("change sound play triggered");
            ...
            if (this.cellArray[i] && this.cellArray[i].parent && cc.rectContainsPoint(this.cellArray[i].getBoundingBox(), this.cellArray[i].parent.convertToNodeSpace(location)) &&  this.cellArray[i].parent.parent && this.cellArray[i].parent.parent.isVisible() ) {
                if(!this.highlightedCell) {
                    ...
                    if (eventType == cc.EventMouse.UP) {
                       // this.highlightedCell.playImg.setTexture(new cc.Sprite(ACTIVITY_CWC_1.spriteBasePath + "Characters_play.png").getTexture());
                        this.cellArray[i].playImg.setTexture(new cc.Sprite(ACTIVITY_CWC_1.spriteBasePath + "Characters_play_hover.png").getTexture());
                        this.highlightedCell = this.cellArray[i];
                       ...
                       // this.highlightedCell.playImg.setTexture(new cc.Sprite(ACTIVITY_CWC_1.spriteBasePath + "Characters_play.png").getTexture());
                        // this.highlightedCell.layer._realColor  = cc.color(255, 255, 255, 255);
                        if(this.cellArray[i].playImg){
    ...
    updateRoomData: function() {
        SocketManager.emitCutomEvent("SingleEvent", {'eventType':HDSocketEventType.UPDATE_ROOM_DATA,'roomId': HDAppManager.roomId ,'data':  {"roomId":HDAppManager.roomId,"roomData": {"activity":  ACTIVITY_CWC_1.config.properties.namespace,"data": "", "activityStartTime" : HDAppManager.getActivityStartTime()} }},null);
    },
    ...
    playAnimation: function(tag, key, index){
        ...
        let action = "_idle_";
        let folder = ACTIVITY_CWC_1.config.charactersData[key].name;
        let name = folder;//.toLowerCase();
        console.log("ACTIVITY_CWC_1.config.charactersData[key][\"animation\"][\"RestPose01\"] ", ACTIVITY_CWC_1.config.charactersData[key]["animation"]["RestPose01"]);
        let frameCount  = ACTIVITY_CWC_1.config.charactersData[key]["animation"]["RestPose01"].frameCount;
        let loop = 1000;
        let actionFolder= "Idle";
        let speed = 0.05;

        // let animation
        let idleAnimFrameCount = frameCount;
        ...
        if(index != undefined && index.toString()){
             action =  "_" + ACTIVITY_CWC_1.config.charactersData[key].audioData[index].animation.toLowerCase() + "_";
             folder = ACTIVITY_CWC_1.config.charactersData[key].name;
             name = folder;//.toLowerCase();
             frameCount  = ACTIVITY_CWC_1.config.charactersData[key]["animation"][ACTIVITY_CWC_1.config.charactersData[key].audioData[index].animation].frameCount;
             loop = 1;
             actionFolder=  ACTIVITY_CWC_1.config.charactersData[key].audioData[index].animation;
             speed = 0.05;

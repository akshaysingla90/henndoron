...
ACTIVITY_CWC_1.CharacterConversationLayer = HDBaseLayer.extend({
        ...
        cc.loader.loadJson("res/Activity/"+ activityName +"/config.json", function (error, config) {
            ...
            for (var i = 0; i < ACTIVITY_CWC_1.config.assets.sections.charactersData.data.length; ++i) {
                for (var j = 0; j < ACTIVITY_CWC_1.config.assets.sections.charactersData.data[i].audioData.length; ++j) {
                    cc.loader.load(ACTIVITY_CWC_1.soundPath + ACTIVITY_CWC_1.config.assets.sections.charactersData.data[i].audioData[j].audio);
                }
            }
            ref.setupUI();
    ...
    characterSetup: function () {
        ...
        for (let index in ACTIVITY_CWC_1.config.assets.sections.charactersData.data) {
            let character = this.addSprite(ACTIVITY_CWC_1.spriteBasePath + ACTIVITY_CWC_1.config.assets.sections.charactersData.data[index].imageName, cc.p(ACTIVITY_CWC_1.config.assets.sections.charactersData.data[index].position.x, ACTIVITY_CWC_1.config.assets.sections.charactersData.data[index].position.y), this);
            let characterTag = index == 0 ? ACTIVITY_CWC_1.Tag.character_1 : (index == 1 ? ACTIVITY_CWC_1.Tag.character_2 : ACTIVITY_CWC_1.Tag.character_3);
            let characterTableView = index == 0 ? ACTIVITY_CWC_1.Tag.character_1_tableView : (index == 1 ? ACTIVITY_CWC_1.Tag.character_2_tableView : ACTIVITY_CWC_1.Tag.character_3_tableView);
            let characterTextStart = index == 0 ? ACTIVITY_CWC_1.Tag.character_1_textStart : (index == 1 ? ACTIVITY_CWC_1.Tag.character_2_textStart : ACTIVITY_CWC_1.Tag.character_3_textStart);
            let characterSelectBtn = index == 0 ? ACTIVITY_CWC_1.Tag.character_1_selectBtn : (index == 1 ? ACTIVITY_CWC_1.Tag.character_2_selectBtn : ACTIVITY_CWC_1.Tag.character_3_selectBtn);
            ...
            if (this.isTeacherView) {
                this.textPanelSetup(character, index, cc.p(character.getPositionX() - character.getContentSize().width * 0.135, character.getPositionY() - character.getContentSize().height * 0.28 - this._size.height * 0.26),
                    characterTableView, characterTextStart, characterSelectBtn, this.playSound);
            }
        }
    },
    ...
    textPanelSetup: function (parent, index, position, scrollViewTag, startTag, selectButtonTag, callback) {
        ...
        middleSprite.finalScale = HDUtility.clampANumber((ACTIVITY_CWC_1.config.assets.sections.charactersData.data[index].audioData.length + 1) * 0.275, 0.15, 1.4)
    ...
    playSound: function (cell) {
        if (cell && cell.getParent() && cell.getParent().isVisible()) {
            ...
            cc.audioEngine.playEffect(ACTIVITY_CWC_1.soundPath + ACTIVITY_CWC_1.config.assets.sections.charactersData.data[key].audioData[index].audio);
    ...
    playCharacterData: function (msg) {
        ...
        cc.audioEngine.playEffect(ACTIVITY_CWC_1.soundPath + ACTIVITY_CWC_1.config.assets.sections.charactersData.data[data.character].audioData[data.audioIdx].audio);
...
ACTIVITY_CWC_1.TextCell = cc.TableViewCell.extend({
    ...
    createUI: function (tableIndex, index, startTag) {
        ...
        if (tableIndex != undefined) {
            ...
            let lable = new cc.LabelTTF(ACTIVITY_CWC_1.config.assets.sections.charactersData.data[tableIndex].audioData[index].text,
